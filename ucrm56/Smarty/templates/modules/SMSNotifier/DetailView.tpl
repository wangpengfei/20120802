...
<tr>
	...
	{if $header eq 'StatusInformation'|getTranslatedString}
    	{assign var=header_tmp value='StatusInformation'}
    {else}
    	{assign var=header_tmp value=$header}
    {/if}
	...
	{if $header neq 'Comments'}
							...
							<div style="float:left;font-weight:bold;"><div style="float:left;"><a href="javascript:showHideStatus('tbl{$header_tmp|replace:' ':''}','aid{$header_tmp|replace:' ':''}','{$IMAGE_PATH}');">
							{if $BLOCKINITIALSTATUS[$header] eq 1}
								<img id="aid{$header_tmp|replace:' ':''}" src="{'activate.gif'|@vtiger_imageurl:$THEME}" style="border: 0px solid #000000;" alt="{$APP.Hide}" title="{$APP.Hide}"/>
							{else}
							<img id="aid{$header_tmp|replace:' ':''}" src="{'inactivate.gif'|@vtiger_imageurl:$THEME}" style="border: 0px solid #000000;" alt="{$APP.Display}" title="{$APP.Display}"/>
							{/if}
								</a></div><b>&nbsp;
						        	{$header}
	  			     			</b></div>
...
{if $header neq 'Comments'}
							{if $BLOCKINITIALSTATUS[$header] eq 1}
							<div style="width:auto;display:block;" id="tbl{$header_tmp|replace:' ':''}" >
							{else}
							<div style="width:auto;display:none;" id="tbl{$header_tmp|replace:' ':''}" >
							{/if}
							<table border=0 cellspacing=0 cellpadding=0 width="100%" class="small">
