...
<tr>
	...
	<tr>
		...
		<td align="left" class="cellText small">
			<select name="smsserver_provider" class="small" onchange="_SMSCongiServerShowReqParams(this);">
				...
				<option {if $SMSSERVERINFO.providertype eq $SMSPROVIDER}selected="true"{/if} value="{$SMSPROVIDER}">{$SMSPROVIDER|getTranslatedString:'SMSNotifier'}</option>
				{/foreach}
			</select>
		</td>
	</tr>
	...
	<tr>
		...
		<td align="left" class="cellText small">
			<input type="radio" class="small" name="smsserver_isactive" value="1" {if $SMSSERVERINFO.isactive}checked=true{/if}> {'LBL_OPEN_LABEL'|getTranslatedString:'Settings'}
			<input type="radio" class="small" name="smsserver_isactive" value="0" {if $SMSSERVERINFO.isactive neq 1}checked=true{/if}> {'LBL_CLOSE_LABEL'|getTranslatedString:'Settings'}
		</td>
	</tr>
	...
	<div id="paramrows_{$SMSPROVIDERPARAMKEY}" {if $SMSSERVERINFO.providertype neq $SMSPROVIDERPARAMKEY}style="display: none;"{/if}>
		...
		<tr>
		<td width="33%" nowrap="nowrap" align="right" class="cellLabel small" style="border: none;"><b>{$SMSPROVIDERPARAM|getTranslatedString:'SMSNotifier'}</b></td>
