...
function showDart(sId,anchorImgId,sImagePath)
{
	oObj = eval(document.getElementById(sId));
	if(oObj.style.display == 'block')
	{
	...
	{
        if($("show_dart").innerHTML == ''){
			$("show_dart").innerHTML = '<img src="themes/images/vtbusy.gif" complete="complete"/>';
			get_dart();
		}
			oObj.style.display = 'block';
			eval(document.getElementById(anchorImgId)).src = 'themes/images/activate.gif';
...
<br>
<table style="padding-bottom: 5px; padding-left: 5px; padding-right: 5px; padding-top: 5px;" width="100%" class="small" border="0" cellSpacing="0" cellPadding="0">
<TBODY>
<TR>
<TD class=dvInnerHeader colSpan=5>
<div style="float:left;">
<a href="javascript:showDart('show_dart','aid_dart','{$IMAGE_PATH}');">
<img id="aid_dart" src="{'inactivate.gif'|@vtiger_imageurl:$THEME}" style="border: 0px solid #000000;" alt="{$APP.Hide}" title="{$APP.Hide}"/>
</a></div>
<B>{$MOD.DART}</B> </TD></TR>
<TR>
<td> 
<br>
<div id="show_dart"></div>
</td>
</TR>
</TBODY>
</table>				
...
{if $header neq 'Comments'}
                ...
                {/if}
					<td class="dvtCellLabel" {if $MODULE eq 'HelpDesk' && $keyfldname eq 'comments'}height="26px"{/if} align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$label}</td>
				{/if}
				{if $EDIT_PERMISSION eq 'yes' && $display_type neq '2' && $keyfldname neq 'summary'}
...
<script type='text/javascript'>
var module = "{$MODULE}";
var s = "{$MOD.Start_Date}";
var e = "{$MOD.End_Date}";
{literal}
function get_dart(){
	var sdate = $('dtlview_'+s).innerHTML;
	var edate = $('dtlview_'+e).innerHTML;
	new Ajax.Request(
    		'index.php',
	        {queue: {position: 'end', scope: 'command'},
        		method: 'post',
                postBody:"module="+module+"&action=GetDart&sdate="+sdate + "&edate=" + edate,
                onComplete: function(response) {
                	$("show_dart").innerHTML=response.responseText;
                }});
}
{/literal}
</script>