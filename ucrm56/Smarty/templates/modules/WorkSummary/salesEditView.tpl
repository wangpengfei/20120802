...
<input type="button" class="crmbutton small save" value="{$MOD.View}" onclick="get_dart()"/>
											</div>
										</td>
									   </tr>
...
{if ($MODULE eq 'Emails' || 'Documents' || 'News') and ($USE_RTE eq 'true')}
...
summary.UNSELECTABLE = 'on';

function get_dart(){
	$("show_dart").innerHTML = '<img src="themes/images/vtbusy.gif" complete="complete"/>';
	if(!dateValidate('startdate',alert_arr.STARTDATE,'OTH')) return false;
	if(!dateValidate('enddate',alert_arr.ENDDATE,'OTH')) return false;
	
	var dateelements=splitDateVal($('jscal_field_startdate').value);	
	var sdate = dateelements[2]+'-'+dateelements[1]+'-'+dateelements[0];
	
	var dateelements=splitDateVal($('jscal_field_enddate').value);	
	var edate = dateelements[2]+'-'+dateelements[1]+'-'+dateelements[0];
	
	if (!dateComparison('enddate',alert_arr.ENDDATE,'startdate',alert_arr.STARTDATE,'GE'))
		return false;
	...
	new Ajax.Request(
	        ...
	        {queue: {position: 'end', scope: 'command'},
                ...
                onComplete: function(response) {
					...
					$("dart_tdimg").innerHTML="<img src='themes/images/close.gif' border='0' complete='complete' onclick=\"$('show_dart').innerHTML=''\"/>";
                }});
}
