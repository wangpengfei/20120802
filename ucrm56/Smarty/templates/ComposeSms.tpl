...
<link REL="SHORTCUT ICON" HREF="include/images/ubicrm_icon.ico">	
...
{literal}
<form name="EditView" method="POST" ENCTYPE="multipart/form-data" action="index.php" onSubmit="if(sms_validate(this.form,'')) { VtigerJS_DialogBox.block();} else { return false; }">
{/literal}
...
<tbody>
   ...
   <tr>
	...
	<td class="cellText" style="padding: 5px;" align="left" nowrap>
		...
		<img src="{'select.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='return window.open("index.php?module="+ document.EditView.parent_type.value +"&action=Popup&html=Popup_picker&form=HelpDeskEditView&popuptype=set_return_smss","test","width=640,height=602,resizable=0,scrollbars=0,top=150,left=200");' align="absmiddle" style='cursor:hand;cursor:pointer'>&nbsp;
	...
	<tr>
	{if 'ccmail'|@smss_checkFieldVisiblityPermission eq '0'}   
   ...
   {if 'bccmail'|@smss_checkFieldVisiblityPermission eq '0'}   
   	<tr>
   ...
   <tr>
	<td colspan="3" class="mailSubHeader" style="padding: 5px;" align="center">
		 <input title="{$APP.LBL_SELECTSMSTEMPLATE_BUTTON_TITLE}" accessKey="{$APP.LBL_SELECTSMSTEMPLATE_BUTTON_KEY}" class="crmbutton small edit" onclick="window.open('index.php?module=Users&action=lookupsmstemplates','smstemplate','top=100,left=200,height=400,width=500,resizable=yes,scrollbars=yes,menubar=no,addressbar=no,status=yes')" type="button" name="button" value=" {$APP.LBL_SELECTSMSTEMPLATE_BUTTON_LABEL}  ">
		<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton small save" onclick="return sms_validate(this.form,'save');" type="button" name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL} " >&nbsp;
		<input name="{$MOD.LBL_SEND}" value=" {$APP.LBL_SEND} " class="crmbutton small save" type="button" onclick="return sms_validate(this.form,'send');">&nbsp;
   ...
   <tr>
	<td colspan="3" class="mailSubHeader" style="padding: 5px;" align="center">
		 <input title="{$APP.LBL_SELECTSMSTEMPLATE_BUTTON_TITLE}" accessKey="{$APP.LBL_SELECTSMSTEMPLATE_BUTTON_KEY}" class="crmbutton small edit" onclick="window.open('index.php?module=Users&action=lookupsmstemplates','smstemplate','top=100,left=200,height=400,width=500,menubar=no,addressbar=no,status=yes')" type="button" name="button" value=" {$APP.LBL_SELECTSMSTEMPLATE_BUTTON_LABEL}  ">
		<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accessKey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton small save" onclick="return sms_validate(this.form,'save');" type="button" name="button" value="  {$APP.LBL_SAVE_BUTTON_LABEL} " >&nbsp;
		<input name="{$MOD.LBL_SEND}" value=" {$APP.LBL_SEND} " class="crmbutton small save" type="button" onclick="return sms_validate(this.form,'send');">&nbsp;
		<input name="{$APP.LBL_CANCEL_BUTTON_TITLE}" accessKey="{$APP.LBL_CANCEL_BUTTON_KEY}" value=" {$APP.LBL_CANCEL_BUTTON_LABEL} " class="crmbutton small cancel" type="button" onClick="window.close()">
	</td>
...
<script>
var cc_err_msg = '{$MOD.LBL_CC_SMS_ERROR}';
var no_rcpts_err_msg = '{$MOD.LBL_NO_RCPTS_SMS_ERROR}';
var bcc_err_msg = '{$MOD.LBL_BCC_SMS_ERROR}';
...
function sms_validate(oform,mode)
{
	...
	//Changes made to fix tickets #4633, # 5111 to accomodate all possible sms formats
	var sms_regex = /^[a-zA-Z0-9]+([\_\-\.]*[a-zA-Z0-9]+[\_\-]?)*@[a-zA-Z0-9]+([\_\-]?[a-zA-Z0-9]+)*\.+([\_\-]?[a-zA-Z0-9])+(\.?[a-zA-Z0-9]+)*$/;
	...
	if(document.EditView.ccmail != null){
		if(document.EditView.ccmail.value.length >= 1){
	    	...
	    	for(var i=0; i<=arr.length-1; i++){
				...
				else if(trim(arr[i]) != "" && !(sms_regex.test(trim(arr[i]))))
	            {
	                    alert(cc_err_msg+": "+arr[i]);
	                    return false;
	            }
	...
	{
		if(sms_sub = prompt('You did not specify a subject from this sms. If you would like to provide one, please type it now','(no-Subject)'))
		{
			oform.subject.value = sms_sub;
		}else
		{
			return false;
		}
