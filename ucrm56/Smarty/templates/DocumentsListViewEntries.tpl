...
<form name="massdelete" method="POST" id="massdelete">
	...
	<!-- List View Master Holder starts -->
                ...
                <table align="center"  cellpadding="0" cellspacing="0" width="100%">
				  <tr>
				    <td width="20%"  valign="top"  style="border:#CCC double 1">
				    <table style="padding-top:40px" style="border:#CCC double 1"> <tr>
                                <td valign="top" >
                                
                                <img style="vertical-align:middle" alt="" src="Image/base.gif" complete="complete"/>
                                <a href="#" onclick="change_folter('all')" title="{$MOD.SHOWALL_FILE}">{$MOD.SHOWALL}</a><br>
                                	
                                {foreach item=folder from=$ALL_FOLDERS name=ff}
                                	{if $smarty.foreach.ff.last}
                                		<img style="vertical-align:middle" alt="" src="Image/joinbottom.gif" complete="complete"/>
                                    {else}
                                		<img style="vertical-align:middle" alt="" src="Image/join.gif" complete="complete"/>
                                    {/if}
                                    <img style="vertical-align:middle" src="Image/page.gif" complete="complete"/>
                                    <a href="#" onclick="change_folter({$folder.folderid})" title="{$folder.description}">{$folder.foldername}</a><br>
								{/foreach}
                                <script>
								var ids_str = "{$FOLDER_IDS}";
								var ids_arr = ids_str.split(',');
								var ids_len = ids_arr.length;
								{literal}
                                function change_folter(sel){
									var val = sel;
									if(val == 'all'){
										for(var i=0;i<ids_len;i++)
										{
											if(document.getElementById(ids_arr[i])){
												document.getElementById(ids_arr[i]).style.display='block';
											}
										}
									}else{
										for(var i=0;i<ids_len;i++)
																	...
																	<tr>
																		<td class="small" align="left" nowrap="nowrap">{$APP.LBL_YOU_CAN_CREATE} {$APP.$vowel_conf}
																			{* vtlib customization: Use translation string only if available *}
									 										...
									 										&nbsp;&nbsp;-<a href="index.php?module={$MODULE}&action=CreateView&return_action=DetailView&parenttab={$CATEGORY}">{$APP.LBL_CREATE} {$APP.$vowel_conf} {$MOD.$MODULE_CREATE}
																		</td>
																	</tr>
																</table>
															{else}
