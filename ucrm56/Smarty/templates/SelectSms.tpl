...
<div id="roleLay" style="z-index:12;display:block;width:400px;" class="layerPopup">
	<table border=0 cellspacing=0 cellpadding=5 width=100% class=layerHeadingULine>
		<tr>
			<td width="90%" align="left" class="genHeaderSmall">{$MOD.SELECT_SMS}
					...
					{/if}
						<div style="height:120px;overflow-y:auto;overflow-x:hidden;" align="center">
							<table border="0" cellpadding="5" cellspacing="0" width="90%">
								{foreach name=smsids key=fieldid item=elements from=$MAILINFO}
								<tr>
									{if $smarty.foreach.smsids.iteration eq 1}	
									<td align="center"><input type="checkbox" checked value="{$fieldid}" name="ssms" /></td>
									{else}
									<td align="center"><input type="checkbox" value="{$fieldid}" name="ssms"  /></td>
									{/if}
									{if $PERMIT eq '0'}
									{if $ONE_RECORD eq 'true'}	
									<td align="left"><b>{$elements.0}</b><br>{$MAILDATA[$smarty.foreach.smsids.index]}</td>
									{else}
