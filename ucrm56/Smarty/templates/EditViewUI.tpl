...
alt="Clear" title="Clear" LANGUAGE=javascript	onClick="this.form.{$fldname}.value=''; this.form.{$fldname}_display.value=''; return false;" align="absmiddle" style='cursor:hand;cursor:pointer'>&nbsp;
		...
		{elseif $uitype eq 23 || $uitype eq 5 || $uitype eq 6}
			...
			<td width="30%" align=left class="dvtCellInfo">
				...
				{/foreach}
				{if $fldname eq 'assigndue_date'}
				   <SELECT id=stdDateFilter class="select small" onchange="showDateRange1(this.options[this.selectedIndex].value ,'{$fldname}')" name=stdDateFilter>
			...
			<td colspan="3" width="30%" align=left class="dvtCellInfo">
					...
					<div id="files_list" style="border: 1px solid grey; width: 500px; padding: 5px; background: rgb(255, 255, 255) none repeat scroll 0%; -moz-background-clip: initial; -moz-background-origin: initial; -moz-background-inline-policy: initial; font-size: x-small">{$APP.Files_Maximum_6}
						...
						{if $maindata[3].0.name neq '' && $DUPLICATE neq 'true'}
						   {foreach name=image_loop key=num item=image_details from=$maindata[3]}
							<div align="center">
								<img src="{$image_details.path}{$image_details.name}" height="50">&nbsp;&nbsp;[{$image_details.orgname}]<input id="file_{$num}" value="{$APP.LBL_DELETE}" type="button" class="crmbutton small delete" onclick='this.parentNode.parentNode.removeChild(this.parentNode);delRowEmt("{$image_details.orgname}")'>
							</div>
				...
				{else}
					...
					{if $maindata[3].0.name != "" && $DUPLICATE neq 'true'}
						<div id="replaceimage">[{$maindata[3].0.orgname}] <a href="javascript:;" onClick="delimage({$ID})">{$APP.LBL_DELETE}</a></div>
					{/if}
				{/if}
			</td>
...
<img tabindex="{$vt_tab}" src="{'select.gif'|@vtiger_imageurl:$THEME}" alt="{$APP.LBL_SELECT}" title="{$APP.LBL_SELECT}" LANGUAGE=javascript onclick='return window.open("index.php?module=Users&action=Popup&form=UsersEditView&form_submit=false&fromlink={$fromlink}&recordid={$ID}","test","width=640,height=603,resizable=0,scrollbars=0");'align="absmiddle" style='cursor:hand;cursor:pointer'>
		...
		{elseif $uitype eq 83} <!-- Handle the Tax in Inventory -->
         ...
         {elseif $uitype eq 999}
			...
			<td width=30% align=left class="dvtCellInfo">
				<input type="password" name="{$fldname}" tabindex="{$vt_tab}" value="{$fldvalue}" tabindex="{$vt_tab}" class=detailedViewTextBox onFocus="this.className='detailedViewTextBoxOn'" onBlur="this.className='detailedViewTextBox'">
			</td>
         {* <!-- //end  --> *}
		{/if}
{php}
 ...
 $sjsStr = '<script language="JavaScript" type="text/javaScript">
  function showDateRange1( type,lab)
  {
   if( type == "" )
   {
    document.getElementById("jscal_field_"+lab).value = "";
   }
   
   if( type == "today" )
   {
    document.getElementById("jscal_field_"+lab).value = "'.getDisplayDate($today).'";
   }';
 ...
 for($i=1; $i<=12; $i++){
  ...
  $sjsStr .= '
    ...
    {
        document.getElementById("jscal_field_"+lab).value = "'.getDisplayDate($date).'";
    }';
