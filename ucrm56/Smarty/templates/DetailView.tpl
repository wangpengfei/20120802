...
<script>
var empty_amount_mandatory = "{'LBL_POTENTIAL_AMOUNT'|getTranslatedString:'Leads'}";
{literal}
...
<tr>
	...
	<td class="showPanelBg" valign=top width=100%>
		...
		<table align="center" border="0" cellpadding="0" cellspacing="0" width="95%">
			<tr><td>		
		 		...
		 		<span class="dvHeaderText">[ {$USE_ID_VALUE} ] {$NAME} -  {$SINGLE_MOD|@getTranslatedString:$MODULE} {$APP.LBL_INFORMATION}</span>&nbsp;&nbsp;&nbsp;<span class="small">{$UPDATEINFO} 
                <input type="button" class="crmbutton small edit" value="{$APP.LBL_RETURN_LIST}" onclick="location.href='index.php?action=ListView&module={$MODULE}&parenttab={$CATEGORY}'" /></span>&nbsp;<span id="vtbusy_info" style="display:none;" valign="bottom"><img src="{'vtbusy.gif'|@vtiger_imageurl:$THEME}" border="0"></span><span id="vtbusy_info" style="visibility:hidden;" valign="bottom"><img src="{'vtbusy.gif'|@vtiger_imageurl:$THEME}" border="0"></span>
		 	</td></tr>
		 </table>			 
		<br>
			...
			<td>
				...
				<tr>
					...
					<td class="dvtTabCache" align="right" style="width:100%">
						...
						{/if}
						{if $CREATE_DUPLICATE eq 'permitted' && $MODULE neq 'Documents'}
						<input title="{$APP.LBL_DUPLICATE_BUTTON_TITLE}" accessKey="{$APP.LBL_DUPLICATE_BUTTON_KEY}" class="crmbutton small create" onclick="DetailView.return_module.value='{$MODULE}'; DetailView.return_action.value='DetailView'; DetailView.isDuplicate.value='true';DetailView.module.value='{$MODULE}'; submitFormForAction('DetailView','CreateView');" type="button" name="Duplicate" value="{$APP.LBL_DUPLICATE_BUTTON_LABEL}">&nbsp;
						{/if}
	...
	{if $header neq 'Comments'}
							...
							{if $BLOCKINITIALSTATUS[$header] eq 1}
								<img id="aid{$header|replace:' ':''}" src="{'activate.gif'|@vtiger_imageurl:$THEME}" style="border: 0px solid #000000;" alt="{$APP.Hide}" title="{$APP.Hide}"/>
							{else}
							<img id="aid{$header|replace:' ':''}" src="{'inactivate.gif'|@vtiger_imageurl:$THEME}" style="border: 0px solid #000000;" alt="{$APP.Display}" title="{$APP.Display}"/>
							{/if}
								</a></div><b>&nbsp;
						        	{$header}
	  			     			</b></div>
...
{if $header neq 'Comments'}
                ...
                {elseif ($MODULE eq 'Accounts' && $keyfldname eq 'accountname') || ($MODULE eq 'Leads' && $keyfldname eq 'company')}
					<td class="dvtCellLabel" align=right width=25%>
                    <input class="crmbutton small cancel" type="button" onclick="fnvshobj(this,'baidu');" value="{$APP.LBL_ONLINE_SEARCH}" />
                    {$label}</td>
                    
<div id="baidu" style="display:none; background-color:#FFF" onmouseover="fnvshNrm('baidu')" onmouseout="fninvsh('baidu')">
<a href="http://www.baidu.com/s?wd={$keyval|@urlencode}&ie=utf-8" target="_blank">{$APP.Baidu}</a><br />
<a href="http://www.google.com.hk/search?hl=zh-CN&q={$keyval|@urlencode}&ie=utf-8" target="_blank">{$APP.Google}</a>
</div>
				{else}
                {if $MODULE eq 'HelpDesk' && $keyfldname eq 'comments'}<tr>
							   			<td colspan=4 class="dvtCellInfo">{$COMMENT_BLOCK}</td>
							   </tr>
                {/if}
                {if $MODULE eq 'Leads' && $keyfldname eq 'track'}<tr>
							   			<td colspan=4 class="dvtCellInfo">{$COMMENT_BLOCK}</td>
							   </tr>
                {/if}
					<td class="dvtCellLabel" {if ($MODULE eq 'HelpDesk' && $keyfldname eq 'comments') || ($MODULE eq 'Leads' && $keyfldname eq 'track')}height="26px"{/if} align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$label}</td>
				{/if}
				{if ($EDIT_PERMISSION eq 'yes' || ($PERMIT_TRACK eq 1 && $keyfldname eq 'track' && $MODULE eq 'Leads')) && $display_type neq '2'}
...
{/if}
		...
		<tr>
			<td class="tagCloudTopBg"><img src="{$IMAGE_PATH}tagCloudName_{php}echo $_SESSION['authenticated_user_language']{/php}.gif" border=0></td>
		</tr>
		<tr>
			...
			<table border=0 cellspacing=0 cellpadding=3 width=100% class="small">
				<tr>
					...
					<td class="dvtTabCacheBottom" align="right" style="width:100%">
						...
						{/if}
						{if $CREATE_DUPLICATE eq 'permitted' && $MODULE neq 'Documents'}
								<input title="{$APP.LBL_DUPLICATE_BUTTON_TITLE}" accessKey="{$APP.LBL_DUPLICATE_BUTTON_KEY}" class="crmbutton small create" onclick="DetailView.return_module.value='{$MODULE}'; DetailView.return_action.value='DetailView'; DetailView.isDuplicate.value='true';DetailView.module.value='{$MODULE}'; submitFormForAction('DetailView','CreateView');" type="submit" name="Duplicate" value="{$APP.LBL_DUPLICATE_BUTTON_LABEL}">&nbsp;
						{/if}
						{if $DELETE eq 'permitted'}
								<input title="{$APP.LBL_DELETE_BUTTON_TITLE}" accessKey="{$APP.LBL_DELETE_BUTTON_KEY}" class="crmbutton small delete" onclick="DetailView.return_module.value='{$MODULE}'; DetailView.return_action.value='index'; {if $MODULE eq 'Accounts'} var confirmMsg = '{$APP.NTC_ACCOUNT_DELETE_CONFIRMATION}' {else} var confirmMsg = '{$APP.NTC_DELETE_CONFIRMATION}' {/if}; submitFormForActionWithConfirmation('DetailView', 'Delete', confirmMsg);" type="button" name="Delete" value="{$APP.LBL_DELETE_BUTTON_LABEL}">&nbsp;
						{/if}
