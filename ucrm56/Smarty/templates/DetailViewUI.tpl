        ...
        {elseif $keyid eq '13' || $keyid eq '104'} <!--Email-->
            <td width=25% class="dvtCellInfo" align="left" id="mouseArea_{$label}" onmouseover="hndMouseOver({$keyid},'{$label}');" onmouseout="fnhide('crmspanid');"><span id="dtlview_{$label}">
				...
				{/if}
                	...
                	{if $keyfldname eq 'yahooid' || $keyfldname eq 'yahoo_id'}
                        {assign var=qq value=$keyval|strpos:"@qq.com"}
                        {if $qq neq ''}
                        <a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin={$keyval}&site=qq&menu=yes">
                        <img border="0" src="http://wpa.qq.com/pa?p=2:{$keyval}:17" align="absmiddle"></a>
                        {elseif $keyval neq ''}
                        <a href="msnim:chat?contact={$keyval}"><img src="http://messenger.services.live.com/users/{$keyval}/presenceimage/" align="absmiddle" border="0"></a>
                        {/if}
                    {/if}
				...
				<div id="internal_mailer_{$keyfldname}" style="display: none;">{$keyfldid}####{$smarty.session.internal_mailer}</div>
					...
					{/foreach}
						...
						{/if}
							...
							{if $MODULE eq 'Documents' || $MODULE eq 'News'}
