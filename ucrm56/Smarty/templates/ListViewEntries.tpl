...
<form name="massdelete" method="POST" id="massdelete" onsubmit="VtigerJS_DialogBox.block();">
     ...
     <input name="current_page_boxes" id="current_page_boxes" type="hidden" value="{$CURRENT_PAGE_BOXES}">
			...
			{elseif $button_check eq 's_mail'}
                ...
                {* Create PDFMaker *}
				{if $BUTTONS neq '' && 'PDFMaker'|@vtlib_isModuleActive neq ''}
                 <input class="crmbutton small edit" type="button" value="{$APP.CREATE_PDF}" onclick="return show_tmps(this,'{$MODULE}')"/>
                 {/if}
                {* Create PDFMaker *}
			...
			<div style="border: 3px solid rgb(153, 153, 153); background-color: rgb(255, 255, 255); width: 45%; position: relative; z-index: 10000000;">
				...
				{if $CHECK.Save eq 'yes' && $MODULE neq 'Emails' && $MODULE neq 'Webmails'}
				...
				<tr>
					...
					{if $MODULE neq 'Calendar'}	
		  			&nbsp;&nbsp;-<a href="index.php?module={$MODULE}&action=CreateView&return_action=DetailView&parenttab={$CATEGORY}">{$APP.LBL_CREATE} {$APP.$vowel_conf}
					{if $MODULE_CREATE eq 'SalesOrder' || $MODULE_CREATE eq 'PurchaseOrder' || $MODULE_CREATE eq 'Invoice' || $MODULE_CREATE eq 'Quotes'}
						 {$MOD.$MODULE_CREATE}
					{else}
						 {* vtlib customization: Use translation string only if available *}
					...
					{else}
					&nbsp;&nbsp;-<a href="index.php?module={$MODULE}&amp;action=CreateView&amp;return_module=Calendar&amp;activity_mode=Events&amp;return_action=DetailView&amp;parenttab={$CATEGORY}">{$APP.LBL_CREATE} {$APP.LBL_AN} {$APP.Event}</a><br>
					&nbsp;&nbsp;-<a href="index.php?module={$MODULE}&amp;action=CreateView&amp;return_module=Calendar&amp;activity_mode=Task&amp;return_action=DetailView&amp;parenttab={$CATEGORY}">{$APP.LBL_CREATE} {$APP.LBL_A} {$APP.Task}</a>
					{/if}
					</td>
				</tr>
				</table> 
...
{if $BUTTONS neq ''}
<input class="crmbutton small edit" type="button" value="{$APP.SHOW_ALL}" onclick="showall('{$MODULE}')"/>
{/if}
