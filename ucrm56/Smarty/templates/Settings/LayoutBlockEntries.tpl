						...
						{if $entries.blockid ne $RELPRODUCTSECTIONID || $entries.blocklabel neq '' }
							...
							<tr>
								...
								<td class="colHeader small"  id = "blockid"_{$entries.blockid} colspan="2" align='right'> 
									...
									{if $MODULE eq 'Users' || $entries.hascustomtable && $entries.blockid neq $COMMENTSECTIONID && $entries.blockid neq $SOLUTIONBLOCKID }
										<img src="{'plus_layout.gif'|@vtiger_imageurl:$THEME}" border="0" style="cursor:pointer;"  onclick="fnvshobj(this,'addfield_{$entries.blockid}'); " alt="Add CustomField" title="{$MOD.LBL_ADD_FIELD}"/>&nbsp;&nbsp;
									{/if}
									...
									{if $entries.blockid neq $COMMENTSECTIONID && $entries.blockid neq $SOLUTIONBLOCKID}
										<img src="{'moveinto.png'|@vtiger_imageurl:$THEME}" border="0"  style="cursor:pointer; height:16px; width:16px" onClick="fnvshobj(this,'movefields_{$entries.blockid}');"  alt="Move Fields" title="{$MOD.LBL_MOVE_FIELDS}"/>&nbsp;&nbsp;
									{/if}
