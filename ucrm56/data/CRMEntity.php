...
class CRMEntity
{
	...
	function saveentity($module,$fileid='')
	{
		...
		if($_REQUEST['return_action'] == 'CallRelatedList' || 
			...
			if($on_focus->IsCustomModule && method_exists($on_focus, 'save_related_module')) {
				$with_module = $module;
				$with_crmid = $this->id;
				if($for_module != $with_module && $for_crmid !=$with_crmid){
	...
	*/
	function uploadAndSaveFile($id,$module,$file_details)
	{
		...
		if($current_language=="zh_cn")
		{
			...
			{
				$binFile = mb_convert_encoding($binFile,'GBK',$default_charset);
			} 
			else 
			{		
				$binFile = iconv($default_charset,'GBK',$binFile);	
			}
		}
  ...
  function insertIntoCrmEntity($module,$fileid='')
  {
	...
	//update wqw
	$wqw_qry    = "SELECT * FROM vtiger_crmentity WHERE crmid=?"; 
	$wqw_result = $adb->pquery($wqw_qry, array($this->id));
	$oldownerid = $adb->query_result($wqw_result,0,"smownerid");
	//end wqw
	...
	if($oldownerid != $ownerid){
		...
		$adb->pquery("INSERT INTO vtiger_owner_change SET crmid='$crmid',oldownerid='$oldownerid',newownerid='$newownerid', operatorid='$operatorid',createdtime='$createdtime'",array());
	}
	//end wqw
   }

//add by wqw
function get_owner_change($id){
	global $log;
	$log->debug("Entering get_owner_change(".$id.") method ...");
	...
	$query = "SELECT 
			oc.*,
			case when (ou.user_name not like '') then ou.user_name else og.groupname end as oldowner,
			case when (nu.user_name not like '') then nu.user_name else ng.groupname end as newowner,
			case when (opu.user_name not like '') then opu.user_name else opg.groupname end as operator
			from vtiger_owner_change oc
			left join vtiger_users ou on ou.id=oc.oldownerid 
			left join vtiger_groups og on og.groupid=oc.oldownerid
			left join vtiger_users nu on nu.id=oc.newownerid 
			left join vtiger_groups ng on ng.groupid=oc.newownerid
			left join vtiger_users opu on opu.id=oc.operatorid 
			left join vtiger_groups opg on opg.groupid=oc.operatorid
			where oc.crmid=? order by createdtime desc";
	$result=$adb->pquery($query, array($id));
	$noofrows = $adb->num_rows($result);
	
	$header[] = $app_strings['LBL_OLDOWNER'];
	$header[] = $app_strings['LBL_NEWOWNER'];
	$header[] = $app_strings['LBL_OPERATOR'];
	$header[] = $app_strings['LBL_CREATETIME'];
	
	while($row = $adb->fetch_array($result))
	{
		$entries = Array();
		$entries[] = '<a target=_blank  href="index.php?module=Users&action=DetailView&record='.$row['oldownerid'].'">'.$row['oldowner'].'</a>';
		$entries[] = '<a target=_blank  href="index.php?module=Users&action=DetailView&record='.$row['newownerid'].'">'.$row['newowner'].'</a>';
		$entries[] = '<a target=_blank  href="index.php?module=Users&action=DetailView&record='.$row['operatorid'].'">'.$row['operator'].'</a>';
		$entries[] = $row['createdtime'];
		$entries_list[] = $entries;
	}
	
	$return_data = Array('header'=>$header,'entries'=>$entries_list);
	
	$log->debug("Exiting get_owner_change method ...");
...
//end wqw	
  ...
  function insertIntoEntityTable($table_name, $module, $fileid='')
  {
	  ...
	  global $adb;
	  $modifier =$current_user->user_name;
	  $insertion_mode = $this->mode;

	  if($_REQUEST['module'] == 'Leads' && $this->column_fields['track'])
	  {
		  $sql = "insert into vtiger_leadstrack values(?,?,?,?)";
		  $params = array($this->id, $this->column_fields['track'],$current_user->user_name, date('Y-m-d H:i:s'));
		  $adb->pquery($sql, $params);
		  $this->column_fields['track'] ='';
	  }
	...
	$tabid= getTabid($module);
	  ...
	  if($insertion_mode == 'edit')
	  {
		  ...
		  elseif($_REQUEST['module'] == 'PurchaseOrder' || $_REQUEST['module'] == 'SalesOrder' || $_REQUEST['module'] == 'Quotes' || $_REQUEST['module'] == 'Invoice')
		  {
			  ...
			  {
				  addInventoryHistory($inventory_module, $this->id,$relatedname,$total,$stat_value,$modifier);
			  }
		  }
	  ...
	  else
	  {
		 ...
		 /*  if($_REQUEST['module'] == 'ModComments')
		  {  $log->fatal($sql1 );   $log->fatal($value );}	 
		 */
...
// Code included by Jaguar - Ends 
  ...
  function retrieve_entity_info($record, $module)
  {
    ...
    foreach($this->tab_name_index as $table_name=>$index)
    {
	    ...
	    if($adb->query_result($result["vtiger_crmentity"],0,"deleted") == 1 || !$adb->query_result($result["vtiger_crmentity"],0,"crmid")) {
	...
	if (isset($this->table_name)) {
    	...
    	if($adb->query_result($result[$this->table_name],0,$mod_index_col) == '')
    		die("<br><br><center>".$record.$module.$app_strings['LBL_RECORD_NOT_FOUND'].
		...
		if($actions) {
			...
			if(in_array('ADD', $actions) && isPermitted($related_module,1, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_ADD_NEW'). " ". getTranslatedString($singular_modname) ."' class='crmbutton small create'" .
					" onclick='this.form.action.value=\"CreateView\";this.form.module.value=\"$related_module\"' type='submit' name='button'" .
