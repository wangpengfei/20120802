...
class Homestuff{
	...
	private function getModuleFilters($sid){
		...
		if(isPermitted($modname,'index') == "yes"){	
			if($adb->num_rows($cvid_check_query)>0){
				...
				if($cvid_check_query->fields['viewname'] == 'Releasev News' && $cvid_check_query->fields['entitytype'] == 'News' ){
					$date = date('Y-m-d');
					$listquery .= " and (vtiger_news .startdate <= '".$date."' and vtiger_news.enddate >='".$date."')";
				}
				...
				{
					$tablename = getTableNameForField($modname,$order_by);
				...
				for($l=0;$l < $column_count;$l++){
					...
					$field_label = getTranslatedString($fldlabel, $modname);
				...
				if(sizeof($header)!=0){
		       		...
		       		echo $app_strings['Fields_not_found_in_Selected_Filter'];
				}
			}
			else{
				echo $app_strings['Fields_not_found_You_have_Selected'];
			}
 		}
		else{
			echo $app_strings['Permission_Denied'];
		}
	}
	...
	private function getRssDetails($rid){
		...
		if(isPermitted('Rss','index') == "yes"){
			...
			echo $app_strings['Not_Accessible'];
		}
		return $return_value;	
	}
	...
	private function getDefaultDetails($dfid,$calCnt){
		...
		elseif($hometype == 'NEWS'){
			require_once "modules/News/GetReleasevNews.php";
			$home_values = GetReleasevNews($maxval, $calCnt);
		}
		/*elseif($hometype == 'LTMSG' && vtlib_isModuleActive('vtmessages')){
			require_once "modules/vtmessages/GetLatestMessages.php";
			$home_values = GetLatestMessages($maxval, $calCnt);
		}*/
	 ...
	 */
 	function getNotebookContents($notebookid){
		...
		if($adb->num_rows($result)>0){
			$contents = nl2br($adb->query_result($result,0,"contents"));
		}
		return $contents;
	}
