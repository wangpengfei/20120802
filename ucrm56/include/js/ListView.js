	...
	if(count > 1) {
		...
		new Ajax.Request(
			...
			{queue:{position:"end", scope:"command"}, 
				method:"post", 
				postBody:"module=PDFMaker&action=get_templates&remodule="+module, 
				onComplete:function (response) {
					var res = response.responseText;
					if(!res){
						alert(alert_arr.EMPTY_TEMPLATE);
						return false;
					}else{
						fnvshobj(obj,'pdf_tmp_show');
						$("pdf_tmp_show_select").innerHTML = '<select style="height:120px;width:265px" multiple id="commontemplateid" name="commontemplateid[]" >'+res+'</select>';
					}
				}
			}
		);
...
function check_pdf_tmp(){
	var template=document.getElementById('commontemplateid');
	var len_src = template.length;
	for(var l=0;l<len_src;l++) { 
		if (template.options[l].selected){
			return true;
		} 
	}
	alert(alert_arr.SELECT); 
...
function change1(obj,divid,str)
{
	var select_options  =  str;
	//Added to remove the semi colen ';' at the end of the string.done to avoid error.
        var x = select_options.split(";");
...
function showDefaultCustomViewTab(selectView,module,parenttab)
{
	...
	new Ajax.Request(
                ...
                {queue: {position: 'end', scope: 'command'},
                       	method: 'post',
                        postBody:"module="+module+"&action="+module+"Ajax&file=ListView&ajax=true&start=1&viewname="+selectView+"&parenttab="+parenttab,
                        onComplete: function(response) {
                        $("status").style.display="none";
                        result = response.responseText.split('&#&#&#');			
					if(module == 'Calendar'){
...
function getListViewEntries_js(module,url)
{
	if(module == 'Documents'){
        ...
        var select_options  =  document.getElementsByName('selected_id'+folder.replace('folderid=',''));
	}else{
