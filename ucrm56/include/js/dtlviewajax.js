                ...
                {queue: {position: 'end', scope: 'command'},
                        ...
                        onComplete: function(response) {
							if(response.responseText.indexOf(":#:FAILURE")>-1) {
								...
								if(module == "Leads" && fieldName == "track") {
									var comments = response.responseText.replace(":#:SUCCESS","");
									if(getObj("track_div") != null) getObj("track_div").innerHTML = comments;
									if(getObj(dtlView) != null) getObj(dtlView).innerHTML = "";
									if(getObj("track") != null) getObj("track").value = "";
								}
	           					$("vtbusy_info").style.display="none";
		...
		if($(temp_fieldname))
		{
			...
			if(internal_mailer_flag == 1){
				...
				if(fieldName == 'yahooid' || fieldName == 'yahoo_id'){
					if(tagValue.indexOf('@qq.com') != -1){
						email_link += '<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin='+tagValue+'&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:'+tagValue+':17" align="absmiddle"></a>';
					}else if(tagValue){
						email_link += '<a href="msnim:chat?contact='+tagValue+'"><img src="http://messenger.services.live.com/users/'+tagValue+'/presenceimage/" align="absmiddle" border="0"></a>';
					}
				}
			}else
