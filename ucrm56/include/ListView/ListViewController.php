			...
			foreach ($listViewFields as $fieldName) {
				...
				if($module == 'Documents' && $fieldName == 'filename') {
					...
					if($_SESSION['internal_mailer'] == 1) {
						...
						if($fieldName == 'yahooid' || $fieldName == 'yahoo_id'){
							if(strpos($rawValue,'@qq.com')){
								$value .= "<a target=\"_blank\"  href=\"http://wpa.qq.com/msgrd?V=1&Uin=$rawValue&Site=ioshenmue&Menu=yes\"><img border=0 src=\"http://wpa.qq.com/pa?p=2:$rawValue:17\" align=\"absmiddle\"></a>";
							}elseif($rawValue){
								$value .= "<a href=\"msnim:chat?contact=$rawValue\"><img src=\"http://messenger.services.live.com/users/$rawValue/presenceimage/\" align=\"absmiddle\" border=0></a>";
							}
						}
					}else {
				...
				//} elseif ($field->getFieldDataType() == 'phone') {
					...
					//	"&quot;$recordId&quot;)'>$value</a>";
				//	$value = "<a href='javascript:SMSNotifierCommon.displaySelectWizard_DetailView(\"$module\", \"$recordId\");'>$value</a>";
					...
					if(!empty($value) && !empty($this->nameList[$fieldName]) && !empty($parentModule)) {
						...
						if ($parentMeta->isModuleEntity() && $parentModule != "Users") {
							...
							if($module == 'ModApprovals' && $fieldName == 'related_to'){
								
								$approvals_status = $this->db->query_result($result, $i, 'approvals_status');
								$reports_to_id = $this->db->query_result($result, $i, 'reports_to_id');
								if($approvals_status == getTranslatedString('PEND','ModApprovals') && $reports_to_id == $current_user->id){
									$value .= "&nbsp;&nbsp;<a href='index.php?module=$parentModule&action=DetailView&record=".
							"$rawValue' title='".getTranslatedString($parentModule,$parentModule)."' style='color:green'><img border=0 src='themes/images/pointer.gif' /> <b>".getTranslatedString('ModApprovals','ModApprovals')."</b></a>&nbsp;&nbsp;";
								}
							}
				...
				if ( in_array($uitype,array(15,16)) ) {
					$value =getTranslatedString($value,$module);// added by wangyefeng for display chinese   in listview  with uitype :15, 16 
				}
				
				if($uitype == 117){
					global $app_currency_strings;
					$value = $app_currency_strings[$value];
				}
