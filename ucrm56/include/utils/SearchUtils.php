...
function BasicSearch($module,$search_field,$search_string){
	...
	//add by wqw  8.2
	elseif($search_field == 'smssourse' && $module == 'SmsLog'){
		$column_name='coursename';
		$table_name='vtiger_course';
		$where="$table_name.$column_name like '". formatForSqlLike($search_string) ."'";
	}
	//end
...
/*			elseif($table_name == "vtiger_course" && $column_name == "linkto1")
			{
				$table_name = "vtiger_department";
...
function getSearch_criteria($criteria,$searchstring,$searchfield)
{
	...
	$where_string = '';
		...
		if($searchfield == 'vtiger_activity.activitytype'){
			switch($searchstring){
...
switch($criteria)
	{
		...
		case 'e':
			global $module;
			if($module == 'SmsLog' && $searchfield == 'vtiger_smslog.smstime')
...
if($currentModule == 'Users' && $tab_col == 'roleid'){
	$tab_col = 'vtiger_role.rolename';
}
	...
	$log->debug("Exiting getWhereCondition method ...");	ob_start();
