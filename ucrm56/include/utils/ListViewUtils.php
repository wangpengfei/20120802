...
function getValue($field_result, $list_result,$fieldname,$focus,$module,$entity_id,$list_result_count,$mode,$popuptype,$returnset='',$viewid='')
{
	...
	elseif($uitype == 13 || $uitype == 104 && ($_REQUEST['action'] != 'Popup' && $_REQUEST['file'] != 'Popup'))
	{
		...
		if($fieldname == 'yahooid'){
			if(strpos($field_val,'@qq.com')){
				$value .= "<a target=\"_blank\"  href=\"http://wpa.qq.com/msgrd?V=1&Uin=$field_val&Site=ioshenmue&Menu=yes\"><img border=0 src=\"http://wpa.qq.com/pa?p=2:{$field_val}:17\" align=\"absmiddle\"></a></a>";
			}elseif($field_val){
				$value .= "<a href=\"msnim:chat?contact={$field_val}\"><img src=\"http://messenger.services.live.com/users/$rawValue/presenceimage/\" align=\"absmiddle\" border=0></a>";
			}
		}
	}
	elseif($uitype == 56)
		...
		if($fieldname == $focus->list_link_field)
		{
			if($mode == "search")
			{
				...
				else
				{
					...
					{
						if(isset($_REQUEST["domkeyid"])&& $_REQUEST["domkeyid"]=="ModApprovalsDetailViewBlockApprovalWidget")//for ModApprovals select approver added by wangyefeng
							
						{
							$domkeyid =$_REQUEST["domkeyid"];
							$value = '<a href="javascript:window.close();" onclick=\'set_return_withdomid("'.$domkeyid.'", "'.$entity_id.'", "'.nl2br(decode_html($slashes_temp_val)).'");\'>'.$temp_val.'</a>';
						}
						else
...
function getListQuery($module,$where='')
{
	...
	Case "Campaigns":
		...
		$query .= "WHERE vtiger_crmentity.deleted = 0 ".$where;
			break;
	Case "Users":
		$query = "SELECT vtiger_users.id,user_name,first_name,last_name,email1,phone_mobile,phone_work,is_admin,status,
					...
					INNER JOIN vtiger_userscf ON vtiger_userscf.id = vtiger_users.id
