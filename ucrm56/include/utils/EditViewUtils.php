...
function getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype,$module_name,$mode='', $typeofdata=null)
{
	...
	global $adb,$log,$default_charset,$default_assign_due_period;
	global $theme;
	global $mod_strings;
	global $app_strings;
	global $current_user;
...
//Added to display the Lead  - Assign due date as default_assign_due_period months future instead of today's date -- 13-10-2010
		 	if($fieldname == 'assigndue_date' && $_REQUEST['module'] == 'Leads')
			{
				$disp_value='';
				/*$addmonth = strtotime("+".$default_assign_due_period." month");
				global $current_user;
				$dat_fmt = (($current_user->date_format == '')?('dd-mm-yyyy'):($current_user->date_format));

				$disp_value = (($dat_fmt == 'dd-mm-yyyy')?(date('d-m-Y',$addmonth)):(($dat_fmt == 'mm-dd-yyyy')?(date('m-d-Y',$addmonth)):(($dat_fmt == 'yyyy-mm-dd')?(date('Y-m-d', $addmonth)):(''))));*/
	...
	elseif($uitype == 55 || $uitype == 255){
		...
		if($uitype == 255 && $fieldpermission == '0'){
			...
			$mclass = new $module_name();
			$sql_field="select * from  vtiger_field where fieldname='salutationtype' and tablename='".$mclass->table_name."'"; 			
			$result_field = $adb->pquery($sql_field,array());
			$salutation_presence = $adb->query_result($result_field,0,"presence");
			if($salutation_presence == 0 || $salutation_presence == 2){
