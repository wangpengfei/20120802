...
function getDetailViewOutputHtml($uitype, $fieldname, $fieldlabel, $col_fields,$generatedtype,$tabid='',$module='')
{
	...
	elseif($uitype == 19)
	{
		if($fieldname == 'notecontent' || $fieldname == 'newscontent'){
			$col_fields[$fieldname]= decode_html($col_fields[$fieldname]);
		}else	
		...
		if($tabid==14)
		{
			...
			}elseif(count($image_array)==1)
			{
				...
				if($current_language=="zh_cn")
				{
					...
					{
						$imgpath1 = mb_convert_encoding($image_orgname_array[0],'GB2312',$default_charset);
					} 
					else 
					{		
						$imgpath1 = iconv($default_charset,'GB2312',$image_orgname_array[0]);	
					}
				}
				list($pro_image_width, $pro_image_height) = getimagesize($imagepath_array[0].$image_id_array[0]."_".$imgpath1);
	...
	elseif($uitype == 105)//Added for user image
	{
		...
		if($image_name != '') {
			...
			if($current_language=="zh_cn")
			{
				if (function_exists("mb_convert_encoding")) 
				{
					$imgpath1 = mb_convert_encoding($imgpath,'GB2312',$default_charset);
				} 
				else 
				{		
					$imgpath1 = iconv($default_charset,'GB2312',$imgpath);	
				}
			}
			list($pro_image_width, $pro_image_height) = getimagesize(decode_html($imgpath1));
...
function getDetailAssociatedProducts($module,$focus)
{
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		if ($entitytype == 'Services') {
			$sc_image_tag = '<a href="index.php?module=ServiceContracts&action=CreateView&service_id='.$productid.'&return_module='.$module.'&return_id='.$focus->id.'">' .
