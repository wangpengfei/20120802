...
function getFieldsListFromQuery($query)
{
	...
	for($i=0; $i < $num_rows;$i++)
	{
		...
		{
			 $fields .= "case when (vtiger_account.accountname not like '') then vtiger_account.accountname else vtiger_contactdetails.lastname end as '".$fieldlabel."',";
		}
		...
		elseif($tablename == 'vtiger_seproductsrel' && $columnName == 'crmid')//Product - Related To
		{
			$fields .= "case vtiger_crmentityRelatedTo.setype 
					when 'Leads' then concat('".getTranslatedString('Leads','Leads')." ::: ',vtiger_ProductRelatedToLead.lastname,' ',vtiger_ProductRelatedToLead.firstname) 
					when 'Accounts' then concat('".getTranslatedString('Accounts','Accounts')." ::: ',vtiger_ProductRelatedToAccount.accountname) 
					when 'Potentials' then concat('".getTranslatedString('Potentials','Potentials')." ::: ',vtiger_ProductRelatedToPotential.potentialname) 
				    End as 'Related To',";
		}
        ...
        {
                 $fields .= "case vtiger_crmentityRelatedTo.setype
                                when 'Accounts' then concat('".getTranslatedString('Accounts','Accounts')." ::: ',vtiger_account.accountname)
			when 'Contacts' then concat('".getTranslatedString('Contacts','Contacts')." ::: ',vtiger_contactdetails.lastname,' ',vtiger_contactdetails.firstname)
                             End as 'Related To',";
        }
		...
		elseif($tablename == 'vtiger_gathers' && $columnName == 'payer')//Contact - Account Name
		{
			$fields .= "case when (vtiger_account.accountid not like '') then concat('".getTranslatedString('Accounts','Accounts')." ::: ',vtiger_account.accountname) else concat('".getTranslatedString('Contacts','Contacts')." ::: ',
vtiger_contactdetails.lastname,' ',vtiger_contactdetails.firstname) End as '" .$fieldlabel."',";
		}
		elseif($tablename == 'vtiger_gathers' && $columnName == 'salesorder_id')//Contact - Account Name
		{
			$fields .= "vtiger_salesorder.subject as '" .$fieldlabel."',";
		}
		elseif($tablename == 'vtiger_charges' && $columnName == 'vendor_id')//Contact - Account Name
		{
			$fields .= "vtiger_vendor.vendorname as '" .$fieldlabel."',";
		}
		elseif($tablename == 'vtiger_charges' && $columnName == 'purchaseorder_id')//Contact - Account Name
		{
			$fields .= "vtiger_purchaseorder.subject as '" .$fieldlabel."',";
		}
		else
		{
