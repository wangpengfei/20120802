...
if(!is_admin($current_user) && (isPermitted($currentModule, 'Export') != 'yes')) {	
	...
	$focus = CRMEntity::getInstance(vtlib_purify($_REQUEST['module']));
	...
	for($i=0; $i<count($fields_array); $i++) {
		$translated_fields_array[$fields_array[$i]] = getTranslatedString($fields_array[$i],$type);
	}

	$smarty->assign("FIELDS",$translated_fields_array);
