		...
		if(getFieldVisibilityPermission($module,$current_user->id,$fld_name) == '0') {
			...
			for($j=0;$j < count($field_values);$j++) {
				...
				if($ui_type ==56) {
					...
					$purchaseorder_id=$field_values[$j][$fld_name];
					if($purchaseorder_id != '') {
						$purchaseorder_name=getPoName($purchaseorder_id);
					}	
					$value_pair['disp_value']=$purchaseorder_name;
				} elseif($ui_type == 52) {
...
function getDuplicateRecordsArr($module)
{
	...
	{			
		...
		for($k=0;$k<count($col_arr);$k++)
		{
            ...
            if($ui_type[$fld_arr[$k]] ==79)
			{
				$purchaseorder_id=$result[$col_arr[$k]];
				if($purchaseorder_id != '')
					{
						$purchaseorder_name=getPoName($purchaseorder_id);
					}	
				$result[$col_arr[$k]]=$purchaseorder_name;	
			}
...
function return_custom_application_language( $language )
{
				...
				if ( file_exists( "cache/application/language/".$language.".lang.php" ) )
				{
