...
class QueryGenerator {
	...
	public function getQuery() {
		...
		if(empty($this->query)) {
			...
			if($this->module == 'ModApprovals'){
				$columns[] = 'approvals_status';
				$columns[] = 'vtiger_modapprovals.reports_to_id';
			}
		...
		foreach ($tableJoinCondition as $fieldName=>$conditionInfo) {
			foreach ($conditionInfo as $tableName=>$condition) {
				if(!empty($tableList[$tableName])) {
					$tableNameAlias = $tableName.$i; // bug fix by weiqingei for: 建视图，多字段与用户有关时，产生error 
					$i++;
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
					...
					foreach($moduleList as $module) {
						...
						if($module == 'Users' && $fieldName == 'creator'){
							$fieldSql .= "vtiger_crmentity.smcreatorid in (select id from vtiger_users where user_name $valueSql)";
						}else{
					...
					if($fieldName == 'birthday' && !$this->isRelativeSearchOperators(
						...
						if($valueSql =='IS NULL'){
							$fieldSql .= "$fieldGlue (".$field->getTableName().'.'.
								$field->getColumnName().' '.$valueSql.' or '.$field->getTableName().'.'.
								$field->getColumnName().'=\'\''.')';
						}elseif($valueSql =='IS NOT NULL'){
							$fieldSql .= "$fieldGlue (".$field->getTableName().'.'.
								$field->getColumnName().' '.$valueSql.' and '.$field->getTableName().'.'.
								$field->getColumnName().'!=\'\''.')';
						}else{
							$fieldSql .= "$fieldGlue ".$field->getTableName().'.'.
...
if($field->getFieldDataType() == 'picklist'){
	$picklist_value = vtlib_getPicklistValues($field->getFieldName());
	foreach($picklist_value as $key_pl => $val_pl){
		$picklist_value_tr[$key_pl] = getTranslatedString($val_pl,$this->module);
	}
	$key_val = array_search($value,$picklist_value_tr);
	if($key_val !== false){
		$value = $picklist_value[$key_val];
	}
}
/*			if($this->module == 'SalesOrder'){
				if($field->getFieldName() == 'sostatus'){
					switch($value){
						case getTranslatedString('Created','SalesOrder'):
							$value = 'Created';
							break;
						case getTranslatedString('Approved','SalesOrder'):
							$value = 'Approved';
							break;
						case getTranslatedString('Delivered','SalesOrder'):
							$value = 'Delivered';
							break;
						case getTranslatedString('Cancelled','SalesOrder'):
							$value = 'Cancelled';
							break;
					}
				}elseif($field->getFieldName() == 'invoicestatus'){
					switch($value){
						case getTranslatedString('AutoCreated','SalesOrder'):
							$value = 'AutoCreated';
							break;
						case getTranslatedString('Created','SalesOrder'):
							$value = 'Created';
						...
						case getTranslatedString('Approved','SalesOrder'):
							$value = 'Approved';
							break;
						case getTranslatedString('Sent','SalesOrder'):
							$value = 'Sent';
							break;
						case getTranslatedString('Credit Invoice','SalesOrder'):
							$value = 'Credit Invoice';
							break;
						case getTranslatedString('Paid','SalesOrder'):
							$value = 'Paid';
							break;
					}
				}
			}elseif($this->module == 'Potentials'){
				if($field->getFieldName() == 'sales_stage'){
					switch($value){
						case getTranslatedString('Closed Won','Potentials'):
							$value = 'Closed Won';
							break;
						case getTranslatedString('Closed Lost','Potentials'):
							$value = 'Closed Lost';
							break;
					}
				}
			}elseif($this->module == 'HelpDesk'){
				if($field->getFieldName() == 'ticketstatus'){
					switch($value){
						case getTranslatedString('Open','HelpDesk'):
							$value = 'Open';
							break;
						case getTranslatedString('In Progress','HelpDesk'):
							$value = 'In Progress';
							break;
						case getTranslatedString('Wait For Response','HelpDesk'):
							$value = 'Wait For Response';
							break;
						case getTranslatedString('Closed','HelpDesk'):
							$value = 'Closed';
							break;
					}
				}
			}elseif($this->module == 'Faq'){
				if($field->getFieldName() == 'faqstatus'){
					switch($value){
						case getTranslatedString('Draft','Faq'):
							$value = 'Draft';
							break;
						case getTranslatedString('Reviewed','Faq'):
							$value = 'Reviewed';
							break;
						case getTranslatedString('Published','Faq'):
							$value = 'Published';
							break;
						case getTranslatedString('Obsolete','Faq'):
							$value = 'Obsolete';
							break;
					}
				}
			}elseif($this->module == 'Quotes'){
				if($field->getFieldName() == 'quotestage'){
					switch($value){
						case getTranslatedString('Created','Quotes'):
							$value = 'Created';
							break;
						case getTranslatedString('Delivered','Quotes'):
							$value = 'Delivered';
							break;
						case getTranslatedString('Reviewed','Quotes'):
							$value = 'Reviewed';
							break;
						case getTranslatedString('Accepted','Quotes'):
							$value = 'Accepted';
							break;
						case getTranslatedString('Rejected','Quotes'):
							$value = 'Rejected';
							break;
					}
				}
			}elseif($this->module == 'PurchaseOrder'){
				if($field->getFieldName() == 'postatus'){
					switch($value){
						case getTranslatedString('Created','PurchaseOrder'):
							$value = 'Created';
							break;
						case getTranslatedString('Approved','PurchaseOrder'):
							$value = 'Approved';
							break;
						case getTranslatedString('Delivered','PurchaseOrder'):
							$value = 'Delivered';
							break;
						case getTranslatedString('Cancelled','PurchaseOrder'):
							$value = 'Cancelled';
							break;
						case getTranslatedString('Received Shipment','PurchaseOrder'):
							$value = 'Received Shipment';
							break;
					}
				}
			}elseif($this->module == 'Invoice'){
				if($field->getFieldName() == 'invoicestatus'){
					switch($value){
						case getTranslatedString('AutoCreated','Invoice'):
							$value = 'AutoCreated';
							break;
						case getTranslatedString('Created','Invoice'):
							$value = 'Created';
							break;
						case getTranslatedString('Approved','Invoice'):
							$value = 'Approved';
							break;
						case getTranslatedString('Sent','Invoice'):
							$value = 'Sent';
							break;
						case getTranslatedString('Credit Invoice','Invoice'):
							$value = 'Credit Invoice';
							break;
						case getTranslatedString('Paid','Invoice'):
							$value = 'Paid';
							break;
					}
				}
			}elseif($this->module == 'ServiceContracts'){
				if($field->getFieldName() == 'contract_status'){
					switch($value){
						case getTranslatedString('Complete','ServiceContracts'):
							$value = 'Complete';
							break;
					}
				}
	...
	public function addUserSearchConditions($input) {
		...
		if($input['searchtype']=='advance') {
			...
			foreach ($conditionList as $index=>$conditionInfo) {
				if($conditionInfo['fieldname'] == 'sales_stage' && $conditionInfo['value']== getTranslatedString('Other','Potentials')){
					$this->addCondition('sales_stage', 'Closed Won','n');
					$this->addConditionGlue(self::$AND);
					$this->addCondition('sales_stage', 'Closed Lost','n');
					$this->addConditionGlue(self::$AND);
				}else{
