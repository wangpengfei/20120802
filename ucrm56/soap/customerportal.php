...
require_once('modules/HelpDesk/language/zh_cn.lang.php');
...
/**	function used to register new  customer portal user (contacts)
...
{
	global $adb,$log;
	$adb->println("Inside customer portal function register_user( $params ).");
	$adb->println($params );
	...
	// check if 
	// $check_available = $adb->pquery("select * from vtiger_portalinfo where user_name=?", array($email));
	...
	$contact->column_fields['phone'] = $phone;

	//$contact->column_fields['company'] = $company;
	//$contact->column_fields['description'] = $description;
	//$contact->column_fields['portal'] = 1;//portal user check;
  
	$userid = getPortalUserid();
	//$from_email = getUserEmailId('id',$userid);
	$contact->column_fields['assigned_user_id']=$userid;	
	//save
	
	$contact->save("Contacts");
  
   	$contactresult = $adb->pquery("select vtiger_contactdetails.contactid from vtiger_contactdetails
		inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_contactdetails.contactid inner join vtiger_contactscf on vtiger_contactscf.contactid = vtiger_contactdetails.contactid 
		where vtiger_crmentity.deleted=0 and vtiger_contactdetails.contactid = ?", array($contact->id));
	if($adb->num_rows($contactresult) == 1)
	{
		$adb->println("Contact  from Portal is saved with id => ".$contact->id);
		$record_array[0]['contactid'] = $adb->query_result($contactresult,0,'contactid');
		return $record_array;
	}
	else
	{
		$adb->println("There may be error in saving the Contact.");
		 return null;

	}
	//----------

	//send password by mail to the register use ()
	/*
	$title ="Customer Portal Login Details ";
    $subject = $title;
	$contents = 'Dear  : '.$ticket->column_fields['lastname'].'<br>';
 
	if ($contact->column_fields['assigned_user_id'])
	$adb->println("Send password mail to the customer(contact) who sign up  the portal ");
	$mail_status = send_mail('Contacts',$email,'',$from_email,$subject,$contents);
...
function get_edit($params){
	...
	global $app_strings, $current_user,$mod_strings, $current_language, $currentModule, $theme;
	...
	$category = getParentTab($module);
	$record = $id;
	
	if(!$id){
		setObjectValuesFromRequest($focus);
		if(isset($focus->column_fields['LinkTo']) && !$focus->column_fields['LinkTo']){
			$focus->column_fields['LinkTo'] = $params['customer_account_id'];
		}
		$focus->column_fields['LinkTo'] = $params['customer_account_id'];
	}else{
		$focus->id = $record;
	...
	global $CustomerPortal,$CustomerPortal_userid;
	$CustomerPortal = true;
	$CustomerPortal_userid = getPortalUserid();
	$return = getBlocks($module, $disp_view, $focus->mode, $focus->column_fields);
	...
	$res =  json_encode(array('data'=>$return,'validationArray'=>$validationArray));

	return $res;
}
function  save_detail($params)
{
	if(!$params[0]['module']){
		return array('#Can Not Find Module!!!');
	}
	global $adb,$log;
	global $current_user, $currentModule;
	
	$module = $params[0]['module'];
	$id		= $params[0]['recordid'];
	...
	$adb->println("Inside customer portal function save_detail( $params ).");
	$adb->println($params );
	
	require_once("modules/$module/$module.php");
	
	$save_module = new $module;
	if($id){
		$save_module->mode = 'edit';
		$save_module->id = $id;
		$save_module->retrieve_entity_info($id, $module);
	}

	$array_keys = array_keys($save_module->column_fields);
	foreach($params[0] as $key => $val){
		if(in_array($key,$array_keys)){
			$save_module->column_fields[$key] = $val;
		}
	}
	...
	if($module == 'Documents'){
		...
		if($default_language == "zh_cn")
		{
			...
			{
				$params[0]['filename'] = mb_convert_encoding($params[0]['filename'],'GB2312','UTF-8');
			}
			else 
			{
				$params[0]['filename'] = iconv('UTF-8','GB2312',$params[0]['filename']);
			}
		}
		
		$upload_file_path = decideFilePath();
		$binFile = sanitizeUploadFileName($params[0]['filename'], $upload_badext);
		$current_id = $adb->getUniqueID("vtiger_crmentity");
		rename('cache/upload/'.$params[0]['filename'],$upload_file_path.$current_id."_".$binFile);
		unlink('cache/upload/'.$params[0]['filename']);
		$save_module->column_fields['filestatus'] = 1;

		$save_module->save('Documents');

		$ownerid = $current_user->id;
			
		$sql1 = "insert into vtiger_crmentity (crmid,smcreatorid,smownerid,setype,description,createdtime,modifiedtime) values(?, ?, ?, ?, ?, ?, ?)";
		$params1 = array($current_id, $current_user->id, $ownerid, 'Documents'." Attachment", '', date('Y-m-d H:i:s'),date('Y-m-d H:i:s'));
		$adb->pquery($sql1, $params1);

		$sql2="insert into vtiger_attachments(attachmentsid, name, description, type, path) values(?, ?, ?, ?, ?)";
		$params2 = array($current_id, $save_module->column_fields['filename'], '', $save_module->column_fields['filetype'], $upload_file_path);
		$result=$adb->pquery($sql2, $params2);

		$query = "delete from vtiger_seattachmentsrel where crmid = ?";
		$qparams = array($save_module->id);
		$adb->pquery($query, $qparams);

		$sql3='insert into vtiger_seattachmentsrel values(?,?)';
		$adb->pquery($sql3, array($save_module->id, $current_id));
	}else{
		$save_module->save($module);
	}
	if($module != 'Accounts' && !$id){
		$accounts = new Accounts();
		$accounts->save_related_module('Accounts', $params[0]['customer_account_id'], $module, $save_module->id);
	}
	return array('module'=>$module,'id'=>$save_module->id);
}
...
function get_list_values($id,$module,$sessionid,$only_mine='true')
{
	...
	else if ($module == 'Documents')
	{
		...
		$query = "select vtiger_performance.*,vtiger_performance.linkto as entityid from vtiger_performance
		 inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_performance.performanceid
		 where vtiger_crmentity.deleted = 0 and vtiger_performance.linkto IN (".generateQuestionMarks($entity_ids_list).")";	
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}else if ($module == 'Keyman'){
		$query = "select vtiger_keyman.*,vtiger_keyman.linkto as entityid from vtiger_keyman
		 inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_keyman.keymanid
		 where vtiger_crmentity.deleted = 0 and vtiger_keyman.linkto IN (".generateQuestionMarks($entity_ids_list).")";	
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}else if ($module == 'Qualification'){
		$query = "select vtiger_qualification.*,vtiger_qualification.linkto as entityid from vtiger_qualification
		 inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_qualification.qualificationid
		 where vtiger_crmentity.deleted = 0 and vtiger_qualification.linkto IN (".generateQuestionMarks($entity_ids_list).")";	
		$params = array($entity_ids_list);
		$fields_list['Related To'] = 'entityid';
	}
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($module == 'Performance'){
				if($fieldname == 'performancename'){
					$fieldid = $adb->query_result($res,$j,'performanceid');
					$fieldvalue ='<a href="index.php?module=Performance&action=index&id='.$fieldid.'">'.$fieldvalue.'</a>';
				}
			}
			if($module == 'Keyman'){
				if($fieldname == 'keymanname'){
					$fieldid = $adb->query_result($res,$j,'keymanid');
					$fieldvalue ='<a href="index.php?module=Keyman&action=index&id='.$fieldid.'">'.$fieldvalue.'</a>';
				}
			}
			if($module == 'Qualification'){
				if($fieldname == 'qualificationname'){
					$fieldid = $adb->query_result($res,$j,'qualificationid');
					$fieldvalue ='<a href="index.php?module=Qualification&action=index&id='.$fieldid.'">'.$fieldvalue.'</a>';
				}
			}
		...
		$output[1][$module]['crmid'][$j] = $fieldid;
	}
...
function get_filecontent_detail($id,$folderid,$module,$customerid,$sessionid)
{
	...
	if($module == 'Documents')
	{
		...
		if($default_language=="zh_cn")
		{
			if (function_exists("mb_convert_encoding"))
			{
				$saved_filename = mb_convert_encoding($saved_filename,'GB2312','UTF-8');
			}
			else 
			{
				$saved_filename = iconv('UTF-8','GB2312',$saved_filename);
			}
		}
...
function get_details($id,$module,$customerid,$sessionid)
{
	...
	if($module == 'Quotes'){
		...
		$query = "SELECT vtiger_performance.*,vtiger_performancecf.*,
			vtiger_crmentity.* FROM vtiger_performance
			INNER JOIN vtiger_crmentity
				ON vtiger_crmentity.crmid = vtiger_performance.performanceid
			INNER JOIN vtiger_performancecf
				ON vtiger_performance.performanceid = vtiger_performance.performanceid" .
		" WHERE vtiger_performance.performanceid = (". generateQuestionMarks($id) .") AND vtiger_crmentity.deleted = 0";
	}
	else if($module == 'Keyman'){
		$query = "SELECT vtiger_keyman.*,vtiger_keymancf.*,
			vtiger_crmentity.* FROM vtiger_keyman
			INNER JOIN vtiger_crmentity
				ON vtiger_crmentity.crmid = vtiger_keyman.keymanid
			INNER JOIN vtiger_keymancf
				ON vtiger_keyman.keymanid = vtiger_keyman.keymanid" .
		" WHERE vtiger_keyman.keymanid = (". generateQuestionMarks($id) .") AND vtiger_crmentity.deleted = 0";
	}
	else if($module == 'Qualification'){
		$query = "SELECT vtiger_qualification.*,vtiger_qualificationcf.*,
			vtiger_crmentity.* FROM vtiger_qualification
			INNER JOIN vtiger_crmentity
				ON vtiger_crmentity.crmid = vtiger_qualification.qualificationid
			INNER JOIN vtiger_qualificationcf
				ON vtiger_qualification.qualificationid = vtiger_qualificationcf.qualificationid" .
		" WHERE vtiger_qualification.qualificationid = (". generateQuestionMarks($id) .") AND vtiger_crmentity.deleted = 0";
	}
...
/*
	if($module == 'Accounts'){
		$recordid = generateQuestionMarks($id);
		$currentModule = 'Accounts';
		$modObj = CRMEntity::getInstance($currentModule);
		$related_array = getRelatedLists($currentModule,$modObj);
		foreach($related_array as $header => $val){
			...
			//print_r($relatedListData);
			$output['relatedlist'][]=array('module'=>$header,'data'=>$relatedListData);
		}
	}
	*/
	return $output;
...
function check_permission($customerid, $module, $entityid) {
	...
	switch($module) {
		...
		case 'Performance' : 	$query = "SELECT vtiger_performance.performanceid FROM vtiger_performance
									INNER JOIN vtiger_crmentity	ON vtiger_crmentity.crmid = vtiger_performance.performanceid
									AND vtiger_crmentity.deleted = 0
									WHERE vtiger_performance.linkto in (". generateQuestionMarks($allowed_contacts_and_accounts) .") 
									AND vtiger_performance.performanceid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
		...
		case 'Keyman' : 	$query = "SELECT vtiger_keyman.keymanid FROM vtiger_keyman
									INNER JOIN vtiger_crmentity	ON vtiger_crmentity.crmid = vtiger_keyman.keymanid
									AND vtiger_crmentity.deleted = 0
									WHERE vtiger_keyman.linkto in (". generateQuestionMarks($allowed_contacts_and_accounts) .") 
									AND vtiger_keyman.keymanid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
		...
		case 'Qualification' : 	$query = "SELECT vtiger_qualification.qualificationid FROM vtiger_qualification
									INNER JOIN vtiger_crmentity	ON vtiger_crmentity.crmid = vtiger_qualification.qualificationid
									AND vtiger_crmentity.deleted = 0
									WHERE vtiger_qualification.linkto in (". generateQuestionMarks($allowed_contacts_and_accounts) .") 
									AND vtiger_qualification.qualificationid = ?";
							$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
