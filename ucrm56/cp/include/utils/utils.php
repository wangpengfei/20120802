...
function checkFileAccess($filepath) {
	...
	if(stripos($realfilepath, $rootdirpath) !== 0) {
		global $app_strings;
		die($app_strings['Access_Retricted_File']);
	}
	return true;
}
...
function getblock_fieldlistview($block_array,$block)
{
	...
	if($nooffields!='')
	{
		...
		$list .= "<td  class='detailedViewHeader' >".getTranslatedString('ACTIVE')."</td>";
		$list .='</tr>';
	}	
	if($noofdata != '')
	{
		for($j=0;$j<$noofdata;$j++)
		{
			...
			$list .= "<td><a href='index.php?module=$block&action=EditView&id=".$block_array[1][$block]['crmid'][$j]."'>".getTranslatedString('EDIT')."</a></td>";
