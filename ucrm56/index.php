...
if(vtlib_isModuleActive('PDFMaker') && isset($_REQUEST['action']) && $_REQUEST['action']=='DetailView'){
  $is_detail_view = true;
  ...
  if(!isset($_SESSION["template_languages"]) || $_SESSION["template_languages"]=="") {
	...
	global $languages;
    $_SESSION["template_languages"] = array($_SESSION['authenticated_user_language']=>$languages[$_SESSION['authenticated_user_language']]);
  }   
}
// ITS4YOU-END
...
if(isset($action) && isset($module))
{
	...
	if(preg_match("/^Save/", $action) ||
		...
		preg_match("/^mailmergedownloadfile/",$action) || preg_match("/^Webmails/",$module) && preg_match("/^get_img/",$action) || preg_match("/^download/",$action) || 
...
if($use_current_login)
{
	...
	if($current_user->page_size){
		global $list_max_entries_per_page;
		$list_max_entries_per_page = $current_user->page_size;
	}
...
if(!$skipSecurityCheck)
{
	...
	if(preg_match('/Ajax/',$action)) {
        ...
        {
			if($_REQUEST['action'] == 'ModApprovalsAjax' || $_REQUEST['action'] == 'ModCommentsAjax'){
				$display = 'yes';
			}else{
                $display = isPermitted($module,$now_action);
			}				
...
if((!$viewAttachment) && (!$viewAttachment && $action != 'home_rss') && $action != 'Tickerdetail' && $action != $Ajx_module."Ajax" && $action != "chat" && $action != "HeadLines" && $action != 'massdelete'  &&  $action != "DashboardAjax" && $action != "ActivityAjax")
{
	...
	
	{
		...
		echo "<tr><td class=small align=left><a href='http://www.ubiwire.com' target='_blank'>友贝科技</a> <a href='http://www.ucrm.net' target='_blank'> 客户关系管理CRM V56 </a></td></tr></table>";
