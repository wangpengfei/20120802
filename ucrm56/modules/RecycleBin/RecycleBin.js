...
function massRestore()
{
	...
	if(confirm(mod_alert_arr.MSG_RESTORE_CONFIRMATION + " " + xx))
	{
		$("status").style.display="inline";
		new Ajax.Request(
       	        'index.php',
...
{
	if(confirm(mod_alert_arr.MSG_RESTORE_THIS))
	{
		$("status").style.display="inline";
		new Ajax.Request(
			'index.php',
...
function callDeleteRecyclebin() {
	...
	document.getElementById('rb_delete_conf_id').style.display = 'block';
}
...
function deleteRecyclebin(id) {
	...
	var selectmodule = $('selected_module').value;
	var idlist =  document.getElementById("allselectedboxes").value;
	var url = "module=RecycleBin&action=RecycleBinAjax&file=EmptyRecyclebin&mode=ajax&ajax=true";
	url += "&idlist="+encodeURIComponent(idlist);
	url += "&selectmodule="+encodeURIComponent(selectmodule);
	new Ajax.Request(
		'index.php',
		{queue: {position: 'end', scope: 'command'},
			method: 'post',
