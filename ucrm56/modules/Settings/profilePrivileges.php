...
if($mode == 'view')
{
	...
	foreach($act_perr_arry as $tabid=>$action_array)
	{
		...
		$entity_name = getTabname($tabid);
		  //Create Permission
		$tab_create_per_id = $action_array['0'];
		$tab_create_per = getDisplayValue($tab_create_per_id,$tabid,'0');
	
		//Edit Permission
		$tab_edit_per_id = $action_array['1'];
		$tab_edit_per = getDisplayValue($tab_edit_per_id,$tabid,'1');
...
if($mode == 'edit')
{
	...
	foreach($act_perr_arry as $tabid=>$action_array)
	{
		...
		//Create Permission
		$tab_create_per_id = $action_array['0'];
		$tab_create_per = getDisplayOutput($tab_create_per_id,$tabid,'0');

	    //Edit Permission
		$tab_edit_per_id = $action_array['1'];
		$tab_edit_per = getDisplayOutput($tab_edit_per_id,$tabid,'1');
...
if($mode == 'create')
{
	...
	else
	{
		...
		foreach($act_perr_arry as $tabid=>$action_array)
		{
			...
			//Create Permission
			$tab_create_per_id = $action_array['0'];
			$tab_create_per = getDisplayOutput(0,$tabid,'0');

			//Edit Permission
			$tab_edit_per_id = $action_array['1'];
			$tab_edit_per = getDisplayOutput(0,$tabid,'1');
...
if($mode=='view')
{
	...
	for($i=0; $i<count($fieldListResult);$i++)
	{
		...
		{
			$language_strings = return_module_language($current_language,'Calendar');
		}
