...
$tmp_name = $_FILES["binFile"]["name"];
if($current_language=="zh_cn")
{
	if (function_exists("mb_convert_encoding"))
	{
		$tmp_name = mb_convert_encoding($tmp_name,'GB2312',$default_charset);
	}
	else 
	{
		$tmp_name = iconv($default_charset,'GB2312',$tmp_name);
	}
}

if(move_uploaded_file($_FILES["binFile"]["tmp_name"],$uploaddir.$tmp_name)) 
{
  ...
  $file_type_value = strtolower($filetype_array[1]);
    ...
    if($filesize != 0)	
    {
		...
		if($current_language=="zh_cn")
		{
			if (function_exists("mb_convert_encoding"))
			{
				$tmp_binFile = mb_convert_encoding($binFile,'GB2312',$default_charset);
			}
			else 
			{
				$tmp_binFile = iconv($default_charset,'GB2312',$binFile);
			}
		}
		
 		$data = base64_encode(fread(fopen($uploaddir.$tmp_binFile, "r"), $filesize));
