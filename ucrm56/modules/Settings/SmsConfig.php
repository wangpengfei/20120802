...
if($_REQUEST['mail_error'] != '') {
	require_once("modules/Smss/mail.php");
	$error_msg = strip_tags(parseSmsErrorString($_REQUEST['mail_error']));
...
$result = $adb->pquery($sql, array('sms'));
...
$from_sms_field = $adb->query_result($result, 0, 'from_sms_field');
...
if(isset($_REQUEST['from_sms_field'])){

	$smarty->assign("FROM_EMAIL_FIELD",vtlib_purify($_REQUEST['from_sms_field']));
}
elseif(isset($from_sms_field)){
	$smarty->assign("FROM_EMAIL_FIELD",$from_sms_field);
}
...
if(isset($_REQUEST['smsconfig_mode']) && $_REQUEST['smsconfig_mode'] != '')
	$smarty->assign("EMAILCONFIG_MODE",vtlib_purify($_REQUEST['smsconfig_mode']));
else
