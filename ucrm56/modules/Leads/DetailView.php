...
$smarty->assign("COMMENT_BLOCK",$focus->getTrackInformation($focus->id));
...
if(isPermitted("Leads","CreateView","") == 'yes')
	$smarty->assign("CREATE_DUPLICATE","permitted");

if(isPermitted("Leads","CreateView",$_REQUEST['record']) == 'yes' && isPermitted("Leads","ConvertLead") =='yes' && (isPermitted("Accounts","CreateView") =='yes' || isPermitted("Contacts","CreateView") == 'yes') && (vtlib_isModuleActive('Contacts') || vtlib_isModuleActive('Accounts')))
{
...
$smarty->assign("WQW_DATE", getCriteriaJS1());
function getCriteriaJS1()
{
 ...
 $sjsStr = '<script language="JavaScript" type="text/javaScript">
  function showDateRange1( type,lab)
  {
    if( type == "" )
   {
    document.getElementById("txtbox_"+lab).value = "";
   }
   if( type == "today" )
   {
    document.getElementById("txtbox_"+lab).value = "'.getDisplayDate($today).'";
   }';
   
 for($i=1; $i<=12; $i++){
  $date = date("Y-m-d",mktime(0, 0, 0, date("m")+$i  , date("d"), date("Y")));
  $sjsStr .= '
    else if( type == "'.$i.'" )
    {
 
         document.getElementById("txtbox_"+lab).value = "'.getDisplayDate($date).'";
    }';
