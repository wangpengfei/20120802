...
$crmid ='';
if(vtlib_isModuleActive('Accounts') && isPermitted("Accounts","CreateView") =='yes'){
...
if(vtlib_isModuleActive('Contacts') && isPermitted("Contacts","CreateView") =='yes'){
	$date_entered = $adb->formatDate(date('Y-m-d H:i:s'), true);
	$date_modified = $adb->formatDate(date('Y-m-d H:i:s'), true);
...
if((! isset($createpotential) || ! $createpotential == "on") && (!empty($potential_name)) && isPermitted("Potentials","CreateView") =='yes' && ($crmid != '' || $crmcontactid != '') && vtlib_isModuleActive('Potentials'))
{
...
function saveLeadRelations($leadid, $relatedid, $setype)
{
	...
	$result = $adb->pquery("select * from vtiger_crmentity where crmid=?", array($leadid));
	$noofproducts = $adb->num_rows($result);
	if( 0 < $noofproducts )
	{
		$adb->pquery("insert into vtiger_crmentityrel values(?,?,?,?)", array($relatedid, $setype, $leadid, 'Leads'));
	}
	/*
	...
	*/
	...
	$log->debug("Exit from function saveLeadRelations.");
}
