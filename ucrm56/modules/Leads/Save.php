...
if(isset($_REQUEST['dup_check']) && $_REQUEST['dup_check'] != '')
{
	...
	$query = "SELECT crmid,createdtime,user_name,last_name 
	...
	$id = $_REQUEST['record'];
	if(isset($id) && $id !='') {
		$query .= " and vtiger_leadaddress.leadaddressid != ?";
	...
	{
		echo $mod_strings['LBL_MOBILE_EXIST']."\n"
		.$app_strings['LBL_ASSIGNED_TO'].': '.$adb->query_result($result,0,'last_name').'['.$adb->query_result($result,0,'user_name')."] \n"
		.$app_strings['LBL_CREATED'].': '.$adb->query_result($result,0,'createdtime');
	}
	else
	{
		echo 'SUCCESS';
