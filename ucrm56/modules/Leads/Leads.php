...
class Leads extends CRMEntity {
	...
	var $list_fields = Array(
		...
		'Mobile'=>Array('leadaddress'=>'mobile'),
	...
	*/
	...
	function getTrackInformation($leadid)
	{
		global $log;
		$log->debug("Entering getTrackInformation(".$leadid.") method ...");
		global $adb;
		global $mod_strings, $default_charset;
		$sql = "select * from vtiger_leadstrack where leadid=?";
		$result = $adb->pquery($sql, array($leadid));
		$noofrows = $adb->num_rows($result);

		//In ajax save we should not add this div
		if($_REQUEST['action'] != 'LeadsAjax')
		{
			$list .= '<div id="track_div" style="overflow: auto;height:200px;width:100%;">';
			$enddiv = '</div>';
		}
		for($i=0;$i<$noofrows;$i++)
		{
			if($adb->query_result($result,$i,'content') != '')
			{
				//this div is to display the comment
				$comment = $adb->query_result($result,$i,'content');
				...
				$list .= $adb->query_result($result,$i,'owner');
					
				$list .= '&nbsp;&nbsp;&nbsp;'.$mod_strings['LBL_TIME'].' : '.$adb->query_result($result,$i,'createdtime').' &nbsp;';
		...
		$log->debug("Exiting getTrackInformation method ...");
		return $list;
	}
