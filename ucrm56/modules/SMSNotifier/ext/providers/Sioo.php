...
//** http://www.sioo.com.cn **//

include_once dirname(__FILE__) . '/../ISMSProvider.php';
include_once 'vtlib/Vtiger/Net/Client.php';

class Sioo implements ISMSProvider {
	
	private $_username;
	private $_password;
	private $_parameters = array();
	
	const SERVICE_URI = 'http://api.52ao.com/';
	...
	public function setAuthParameters($username, $password) {
		...
		$this->_password = strtolower(md5($password));
	}
	...
	public function send($message, $tonumbers, $time) {
		...
		$response = $httpClient->doPost(array(
			...
			'content' => mb_convert_encoding($message,'GBK','GB2312,UTF-8')
		));
	...
	public function get_overage($username, $password){
		...
		$pwd = strtolower(md5($password));
		
		$httpClient = new Vtiger_Net_Client('http://api.52ao.com/m/');
		$response = $httpClient->doPost(array(
			'user' => $username,
			'pass' => $password
		));
