...
class SMSNotifier extends SMSNotifierBase {
	...
	static function checkServer() {
		...
		if($result && $adb->num_rows($result)) {
			...
			if(!empty($resultrow['username'])&& !empty($resultrow['password'])){
				return true;
			}else{
				return false;
			}
	 ...
	 */
	static function sendsms($message, $tonumbers, $ownerid = false, $linktoids = false, $linktoModule = false, $send_time) {
		...
		$focus->column_fields['schedule_time'] = $send_time?$send_time:date('Y-m-d H:i');
		...
		$responses = self::fireSendSMS($message, $tonumbers, $send_time);
	...
	static function fireSendSMS($message, $tonumbers, $send_time) {
		global $log;
		$provider = SMSNotifierManager::getActiveProviderInstance();
		if($provider) {
			return $provider->send($message, $tonumbers, $send_time);
		}
	}
	...
	static function listConfiguredServers() {
		...
		if($result) {
			while($resultrow = $adb->fetch_row($result)) {
				if($resultrow['providertype']){
					$profile = dirname(__FILE__) . '/ext/providers/'.$resultrow['providertype'].'.php';
					if(is_file($profile)){
						$cont = file_get_contents($profile);
						preg_match('/\/\*\* .* \*\*\//',$cont,$return);
						$website = substr($return[0],4,-4);
						if($website){
							$resultrow['providertype'] = '<a href="'.$website.'" target="_blank" >'.getTranslatedString($resultrow['providertype'],'SMSNotifier').'</a>';
						}
					}
				}
				if($resultrow['isactive'] && $resultrow['providertype']){
					$provider = SMSNotifierManager::getActiveProviderInstance();
					if(is_object($provider)){
						$access_num = $provider->get_overage($resultrow['username'],$resultrow['password']);
						$resultrow['providertype'] .= "( $access_num )";
					}
				}
				$servers[] = $resultrow;
			}
