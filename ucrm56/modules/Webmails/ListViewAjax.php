		...
		while ($i<=$c) {
			if(is_array($searchlist)) {
				for($l=0;$l<$num_searches;$l++) {
					if($mails[$start_message]->msgno && $mails[$start_message]->msgno == $searchlist[$l])
...
if (is_array($list)) {
	...
	foreach ($list as $key => $val) {
		...
		$i++;
/*
		$gbkval = getTranslatedString($tmpval,'Webmails');
		if($gbkval == $tmpval){
			$gbkval = mb_convert_encoding($tmpval, 'UTF-8', 'UTF7-IMAP');
			$tmpval = $gbkval;
		}
		*/
		if($_REQUEST["mailbox"] == '')
		 ...
		 */
			...
			if($tmpval[0] != ".")
			{
				...
				$folders .= '<li style="padding-left:0px;"><img src="themes/images/'.$img.'"align="absmiddle" />&nbsp;&nbsp;<a href="javascript:changeMbox(\''.$tmpval.'\');" class="small">'.my_translate($tmpval).'</a>&nbsp;&nbsp;<span id="'.str_replace(' ','_',$tmpval).'_count" style="font-weight:bold">';
				if($unread_msgs > 0)
					$folders .= '(<span id="'.str_replace(' ','_',$tmpval).'_unread">'.$unread_msgs.'</span>)</span>&nbsp;&nbsp;<span id="remove_'.$tmpval.'" style="position:relative;display:none">Remove</span></li>';
				else
					$folders .='</span></li>';
			}
			...
			if($tmpval[0] != ".")
			{
				...
				$boxes .= '<option value="'.$tmpval.'">'.my_translate($tmpval);
				$folders .= '<li ><img src="themes/images/'.$img.'" align="absmiddle" />&nbsp;&nbsp;<a href="javascript:changeMbox(\''.$tmpval.'\');" class="small">'.my_translate($tmpval).'</a>&nbsp;<span id="'.$tmpval.'_count" style="font-weight:bold">';
				if($box->unseen > 0)
