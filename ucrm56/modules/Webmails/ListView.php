		...
		foreach ($list as $key => $val) {
			...
			if ($_REQUEST["mailbox"] == $tmpval) {
				...
				if($tmpval[0] != ".") {
					...
					$folders .= '<li ><img src="themes/images/'.$img.'" align="absmiddle" />&nbsp;&nbsp;<a href="javascript:changeMbox(\''.$tmpval.'\');" class="small">'.my_translate($tmpval).'</a>&nbsp;<span id="'.str_replace(' ','_',$tmpval).'_count" style="font-weight:bold">';
					if($box->unseen > 0)
						$folders .= '(<span id="'.str_replace(' ','_',$tmpval).'_unread">'.$box->unseen.'</span>)</span></li>';
					else
						$folders .='</span></li>';
				}
			}
