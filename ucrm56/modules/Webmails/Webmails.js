...
function OpenComposer(id,mode)
{
        switch(mode)
                ...
                case 'full_view':
                        ...
                        break;
				case 'print':
						url = 'index.php?module=Emails&action=EmailsAjax&file=PrintEmail&record='+id+'&mailbox='+encodeURIComponent(mailbox)+'&print=true';
						break;
                }
...
function runEmailCommand(com,id) {
	...
	$("status").style.display="block";
        new Ajax.Request(
                ...
                {queue: {position: 'end', scope: 'command'},
                        method: 'post',
                        postBody: 'module=Webmails&action=WebmailsAjax&start='+start+'&command='+command+'&mailid='+id+'&file='+file+'&mailbox='+encodeURIComponent(mailbox)+qry_str,
                        onComplete: function(t) {
							var tmp_mailbox = mailbox.replace(' ','_');
                                resp = t.responseText;
								id=resp;
                                ...
                                switch(command) {
									case 'reload':
										...
										var unread_count = parseInt($(tmp_mailbox+"_tempcount").innerHTML);
										if(unread_count > 0) {
											$(tmp_mailbox+"_unread").innerHTML = unread_count;
										}
										else{
											$(tmp_mailbox+"_unread").innerHTML = "";
										}
										...
										$("navTemp").innerHTML = '';
										$(tmp_mailbox+"_tempcount").innerHTML = "";*/
										break;
                                    case 'expunge':
...
function changeMbox(box) {
	...
	change_box=1;
	runEmailCommand("reload",0);
   // location.href = "index.php?module=Webmails&action=index&mailbox="+encodeURIComponent(box);
}
