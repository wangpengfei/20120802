...
class SalesOrder extends CRMEntity {
	...
	var $sortby_fields = Array('subject','smownerid','accountname','lastname','sostatus');		
	...
	{	
		...
		while($row = $adb->fetch_array($result))
		{
			...
			$entries[] = $row['total'];
			$sostatus_tmp =(in_array($row['sostatus'], $sostatus_array))? $row['sostatus']: $error_msg;
			$entries[] = getTranslatedString ($sostatus_tmp);
			$entries[] = getDisplayDate($row['lastmodified']);
			$entries[] = $row['modifier'];
