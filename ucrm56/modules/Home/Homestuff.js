			...
			{queue: {position: 'end', scope: 'command'},
				...
				onComplete: function(response){
					...
					if(response.responseText.indexOf('SUCCESS') > -1){
						...
						$('seqSettings').innerHTML= '<table cellpadding="10" cellspacing="0" border="0" width="100%" class="vtResultPop small"><tr><td align="center">'+alert_arr['Widget_deleted_sucessfully']+'.</td></tr></table>';
...
function frmValidate(){
	if(trim($('stufftitle_id').value)==""){
		alert(alert_arr.EMPTY_WIN_TITLE);
		$('stufftitle_id').focus();
		return false;
	}
	if($('stufftype_id').value=="RSS"){			
		if($('txtRss_id').value==""){
			alert(alert_arr.EMPTY_RSS_URL);
			$('txtRss_id').focus();
			return false;
		}
	}
	...
	if($('stufftype_id').value=="Module"){
		...
		if(cnt>2){
			alert(alert_arr.SELECT_TWO_FIELDS);
			selVal.focus();
			return false;
		}else{
		...
		{queue: {position: 'end', scope: 'command'},
			...
			onComplete: function(response){
				...
				if(!response.responseText){
					alert(alert_arr.UNABLE_ADD_HOMESTUFF);
					$('vtbusy_info').style.display="none";
					$('stufftitle_id').value='';
					$('txtRss_id').value='';
					return false;
