	...
	if($adb->num_rows($result)==0){
	  ...
	  die;
		...
		for($i=0;$i<$adb->num_rows($result);$i++){
			...
			if($viewname == 'All'){
				$viewname = $app_strings['LBL_ALL'];
			}
			if($adb->query_result($result,$i,'userid')==$current_user->id || $adb->query_result($result,$i,"viewname")=='All'){
				$html .= "<option value='".$adb->query_result($result,$i,'cvid')."'>".$viewname."</option>";
			}else{
				$html .= "<option value='".$adb->query_result($result,$i,'cvid')."'>".$viewname."[".$adb->query_result($result,$i,'user_name')."]</option>";
			}
		}
		$html .= '</select>';
	}
...
if(!empty($dash)){
	...
	$graph_array = array("leadsource" => $dashbd_strings['leadsource'],
						  ...
						  "accountstatistics"=> $dashbd_strings['accountstatistics'],
						  "leadsstatistics"=> $dashbd_strings['leadsstatistics'],);
...
if(!empty($_REQUEST['matrixsequence'])){
	...
	echo "<table cellpadding='10' cellspacing='0' border='0' width='100%' class='vtResultPop small'><tr><td align='center'>".$mod_strings['Layout Saved']."</td></tr></table>";
}
