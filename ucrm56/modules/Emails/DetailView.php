...
if(isset($_REQUEST['record'])) 
{
	...
	$to_email = $json->decode(htmlspecialchars_decode($adb->query_result($result,0,'to_email')));
	$cc_email = $json->decode(htmlspecialchars_decode($adb->query_result($result,0,'cc_email')));
	$smarty->assign('TO_MAIL',vt_suppressHTMLTags(@implode(',',$to_email)));	
	$smarty->assign('CC_MAIL',vt_suppressHTMLTags(@implode(',',$cc_email)));	
    $bcc_email = $json->decode(htmlspecialchars_decode($adb->query_result($result,0,'bcc_email')));	
