...
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='')
{
   ...
   /* if(empty($mail->Host)) {
	...
	global $current_user;
	$query = "select cf_614 as SMTP,cf_617 as PWD from vtiger_userscf where id=?";
	$params = array($current_user->id);
	$result = $adb->pquery($query,$params);

	$hostname = $result->fields['SMTP'];
	$password = $result->fields['PWD'];
	$username = $current_user->email1;
...
function addAllAttachments($mail,$record)
{
	...
	for($i=0;$i<$count;$i++)
	{
		...
		if($current_language=="zh_cn")
		{
			...
			{
				$filewithpath = mb_convert_encoding($filewithpath,'GBK',$default_charset);
			} 
			else 
			{		
				$filewithpath = iconv($default_charset,'GBK',$filewithpath);	
			}
		}
