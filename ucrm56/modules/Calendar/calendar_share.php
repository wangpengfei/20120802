...
<table border=0 cellspacing=0 cellpadding=5 width=95% align=center> 
	<tr>
		...
		<td align="left" width="90%">
            ...
            <input type="checkbox" name="entime_check" <?php if($current_user->end_hour != ''){?> checked <?php } ?> onClick="enableCalendtime();">&nbsp;<?php echo $mod_strings['LBL_CALEND']?> 
			<select name="end_hour" <?php if($current_user->start_hour == ''){?>disabled <?php } ?> >
				<?php
					for($i=0;$i<=23;$i++)
					{
						if($i == 0)
						...
						if($i <= 9 && strlen(trim($i)) < 2)
						{
							...
							$value = $i.':00';
							if($value === $current_user->end_hour)
                                                       	        $selected = 'selected';
                                                         else
                                                                $selected = '';
				?>
