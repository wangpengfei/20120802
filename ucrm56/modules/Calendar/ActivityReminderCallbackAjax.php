...
if($active_res!='None'){
	...
	if($reminder_next_time <= $cur_time)
	{
		...
		//add by wqw
		if(vtlib_isModuleActive('ModApprovals'))
		{
			$PEND = getTranslatedString('PEND','ModApprovals');
			$apprval_query = "SELECT vtiger_modapprovals.approvalcontent,vtiger_modapprovals.related_to,vtiger_crmentity2.setype 
			FROM vtiger_modapprovals 
			INNER JOIN vtiger_crmentity ON vtiger_modapprovals.modapprovalsid = vtiger_crmentity.crmid 
			LEFT JOIN vtiger_users ON vtiger_modapprovals.reports_to_id = vtiger_users.id 
			INNER JOIN vtiger_crmentity vtiger_crmentity2 ON vtiger_modapprovals.related_to = vtiger_crmentity2.crmid  AND vtiger_crmentity2.deleted=0
			WHERE vtiger_crmentity.deleted=0 
			AND (( vtiger_users.user_name = '".$current_user->user_name."') 
			AND ( vtiger_modapprovals.approvals_status = '".$PEND."') )";
	
			$result = $adb->query($apprval_query);
			$cbrows = $adb->num_rows($result);
			if($cbrows > 0) {
				for($index = 0; $index < $cbrows; $index++) {
					$approvalcont = $adb->query_result($result, $index, "approvalcontent");
					$related_to = $adb->query_result($result, $index, "related_to");
					$remodule = $adb->query_result($result, $index, "setype");
					
					if(vtlib_isModuleActive($remodule)){
						$pend_approval[] = array('approvalcont'=>$approvalcont,'related_to'=>$related_to,'remodule'=>$remodule);
					}
				}
				
				if(!empty($pend_approval)){
					$smarty->assign("TITLE", $PEND.getTranslatedString('ModApprovals','ModApprovals'));
					$smarty->assign("THEME", $theme);
					$smarty->assign("Pend_Approval", $pend_approval);
					$smarty->assign("APP", $app_strings);
					$smarty->assign("popupid", 'ApprovalReminderCallback');
					$smarty->display("ApprovalReminderCallback.tpl");
				}
			}
		}
		//end by wqw
