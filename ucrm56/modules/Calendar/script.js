...
function enableCalendtime()
{
	if(document.SharingForm.entime_check.checked == true)
		document.SharingForm.end_hour.disabled = false;
	else	
		document.SharingForm.end_hour.disabled = true;
}
function check_form()
{
	formSelectColumnString('inviteesid','selectedusers');
		...
		if(!dateValidate('date_start',alert_arr.Start_Date,'OTH'))
		{
			return false;
		}
		if(!dateValidate('due_date',alert_arr.End_Date,'OTH'))
		{
			return false;
		}
		if(dateComparison('due_date',alert_arr.End_Date,'date_start',alert_arr.Start_Date,'GE'))
		{
			...
			if(!compareDates(date2,alert_arr.START_TIME,date1,alert_arr.END_TIME,'GE')) return false;
            ...
            {
                    if(!dateValidate('followup_date',alert_arr.Followup_Date,'OTH'))
                    {
                            return false;
                    }
                    if(followupformat != '')
					...
					if ( compareDates(date3,alert_arr.Followup_Date,date2,alert_arr.End_Date,'GE'))
                    {
                            if (date3 <= date2)
                            {
