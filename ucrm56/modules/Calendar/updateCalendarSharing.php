...
{
	$sql = "update vtiger_users set start_hour=?,end_hour=? where id=?";
	$adb->pquery($sql, array($_REQUEST['start_hour'],$_REQUEST['end_hour'], $current_user->id));
}
elseif($_REQUEST['start_hour'])
{
	$sql = "update vtiger_users set start_hour=? where id=?";
	$adb->pquery($sql, array($_REQUEST['start_hour'], $current_user->id));
}
elseif($_REQUEST['end_hour'])
{
	$sql = "update vtiger_users set end_hour=? where id=?";
	$adb->pquery($sql, array($_REQUEST['end_hour'], $current_user->id));
}
