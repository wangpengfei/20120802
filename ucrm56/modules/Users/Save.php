...
{
		global $USERS_NUM_QUOTA;
		$USERS_NUM_QUOTA = $USERS_NUM_QUOTA ? $USERS_NUM_QUOTA : 50;
        $users_query = "SELECT * FROM vtiger_users ";
        $users_result = $adb->pquery($users_query, array( ));
		if($adb->num_rows($users_result) >= $USERS_NUM_QUOTA) {
			echo $mod_strings['USERS_NUM_OVERRUN'];
			die;
		}
    ...
    $user_query = "SELECT user_name FROM vtiger_users WHERE user_name =? AND id != ?";
	$user_result = $adb->pquery($user_query, array($_REQUEST['user_name'],$_REQUEST['record']));
    $group_query = "SELECT groupname FROM vtiger_groups WHERE groupname =? AND groupid != ?";
	$group_result = $adb->pquery($group_query, array($_REQUEST['user_name'],$_REQUEST['record']));

	$location = "index.php?module=Users&action=DetailView&parenttab=Settings&record=".vtlib_purify($_REQUEST['record']);

	if($adb->num_rows($user_result) > 0) {
		$error_str = $mod_strings['LBL_USERNAME_EXIST'];
	} elseif($adb->num_rows($group_result) > 0) {
		$error_str = $mod_strings['LBL_GROUPNAME_EXIST'];
	}
	
	if ($error_str != '') {	
		//header("Location:$location&mail_error=".urlencode($error_str));
		header("Location: index.php?action=ListView&module=Users&record=333&parenttab=Settings&user=".$_REQUEST['user_name']."&has_user_error=".urlencode($error_str));
		//header("Location: index.php?action=Error&module=Users&error_string=".urlencode($error_str));
		exit;
	}
}
