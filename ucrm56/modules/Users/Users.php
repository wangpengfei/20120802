...
class Users {
	...
	var $tab_name = Array('vtiger_users','vtiger_userscf','vtiger_attachments','vtiger_user2role','vtiger_asteriskextensions');	
	var $tab_name_index = Array('vtiger_users'=>'id','vtiger_userscf'=>'id','vtiger_attachments'=>'attachmentsid','vtiger_user2role'=>'userid','vtiger_asteriskextensions'=>'userid');
	var $customFieldTable = Array('vtiger_userscf', 'id');
	...
	var $homeorder_array = array('HDB','ALVT','PLVT','QLTQ','CVLVT','HLT','OLV','GRT','OLTSO','ILTI','MNL','OLTPO','LTFAQ', 'UA', 'PA','NEWS','LTMSG');
	...
	function insertIntoEntityTable($table_name, $module)
	{
		...
		for($i=0; $i<$noofrows; $i++)
		{
			...
			{
				$fldvalue = $_REQUEST[$fieldname] ? $_REQUEST[$fieldname] : '';
			}
			if($fldvalue=='') {
	...
	function uploadAndSaveFile($id,$module,$file_details)
	{
	    ...
	    if($current_language=="zh_cn")
		{
			...
			{
				$binFile = mb_convert_encoding($binFile,'GB2312',$default_charset);
			} 
			else 
			{		
				$binFile = iconv($default_charset,'GB2312',$binFile);	
			}
		}
	...
	function getHomeStuffOrder($id){
		global $adb;
		$this->homeorder_array = array('UA', 'PA', 'ALVT','HDB','PLVT','QLTQ','CVLVT','HLT','GRT','OLTSO','ILTI','MNL','OLTPO','LTFAQ','NEWS','LTMSG');
		$return_array = Array();
		$homeorder=Array();
		if($id != ''){
	...
	function insertUserdetails($inVal)
	{
		...
		$visibility=$this->getDefaultHomeModuleVisibility('NEWS',$inVal);
		$sql="insert into vtiger_homestuff values(?,?,?,?,?,?)";
		$res=$adb->pquery($sql, array($s16,16,'Default',$uid,$visibility,'Releasev News'));
		
		$s17=$adb->getUniqueID("vtiger_homestuff");
		$visibility=$this->getDefaultHomeModuleVisibility('LTMSG',$inVal);
		$sql="insert into vtiger_homestuff values(?,?,?,?,?,?)";
		$res=$adb->pquery($sql, array($s17,17,'Default',$uid,$visibility,'Latest Messages'));
		...
		$sql="insert into vtiger_homedefault values(".$s16.",'NEWS',5,'Releasev News')";
		$adb->query($sql);
		
		$sql="insert into vtiger_homedefault values(".$s17.",'LTMSG',5,'Latest Messages')";
		$adb->query($sql);
	}
