	...
	if($adb->num_rows($tktresult))
	{
		...
		for ($i=0; $i<$adb->num_rows($tktresult); $i++)
		{
			...
			if($viewstatus == 'Unread')
				$value[]= '<a style="color:red;" href="index.php?action=DetailView&module=HelpDesk&record='.mb_substr($adb->query_result($tktresult,$i,"ticketid"),0,20,$default_charset).'">'.$adb->query_result($tktresult,$i,"title").'</a>';
			elseif($viewstatus == 'Marked')
				$value[]= '<a style="color:yellow;" href="index.php?action=DetailView&module=HelpDesk&record='.mb_substr($adb->query_result($tktresult,$i,"ticketid"),0,20,$default_charset).'">'.$adb->query_result($tktresult,$i,"title").'</a>';
			else
				$value[]= '<a href="index.php?action=DetailView&module=HelpDesk&record='.mb_substr($adb->query_result($tktresult,$i,"ticketid"),0,20,$default_charset).'">'.mb_substr($adb->query_result($tktresult,$i,"title"),0,20,$default_charset).'</a>';
