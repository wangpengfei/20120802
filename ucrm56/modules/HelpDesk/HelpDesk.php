		...
		if($actions) {
			...
			if(in_array('ADD', $actions) && isPermitted($related_module,1, '') == 'yes') {
				$button .= "<input title='".getTranslatedString('LBL_NEW'). " ". getTranslatedString('LBL_TODO', $related_module) ."' class='crmbutton small create'" .
					" onclick='this.form.action.value=\"CreateView\";this.form.module.value=\"$related_module\";this.form.return_module.value=\"$this_module\";this.form.activity_mode.value=\"Task\";' type='submit' name='button'" .
...
case when (vtiger_users.user_name not like '') then vtiger_users.user_name else vtiger_groups.groupname end as user_name
	...
	function constructUpdateLog($focus, $mode, $assigned_group_name, $assigntype)
	{
		...
		{
			$updatelog = $app_strings['LOG_TICKET_CREATE'];

			if(!empty($assigned_group_name) && $assigntype == 'T')
			{
				$updatelog .= $app_strings['LOG_TICKET_GROUP'].(is_array($assigned_group_name)? $assigned_group_name[0] : $assigned_group_name);
			}
			elseif($focus->column_fields['assigned_user_id'] != '')
			{
				$updatelog .= $app_strings['LOG_TICKET_USER'].getUserName($focus->column_fields['assigned_user_id']);
			}
			else
			{
				$updatelog .= $app_strings['LOG_TICKET_USER'].getUserName($current_user->id);
			}

			$fldvalue = date('Y-m-d H:i:s').' '.$current_user->user_name;
			$updatelog .= " -- ".$fldvalue."--//--";
		}
		else
		{
			...
			if($focus->column_fields['assigned_user_id'] != $old_owner_id)
			{
				...
				if($assigntype == 'T')
					$updatelog .= $app_strings['LOG_TRANS_TO_GROUP'].$owner_name;
				else
					$updatelog .= $app_strings['LOG_TRANS_TO_USER'].decode_html($owner_name); // Need to decode UTF characters which are migrated from versions < 5.0.4.
			}
			//Status change log
			if($old_status != $focus->column_fields['ticketstatus'] && $focus->column_fields['ticketstatus'] != '')
			{
				$updatelog .= $app_strings['LOG_STATUS_CHANGE'].$focus->column_fields['ticketstatus'];
			}
			//Priority change log
			if($old_priority != $focus->column_fields['ticketpriorities'] && $focus->column_fields['ticketpriorities'] != '')
			{
				$updatelog .= $app_strings['LOG_PROIRITY_CHANGE'].$focus->column_fields['ticketpriorities'];
			}
			//Severity change log
			if($old_severity != $focus->column_fields['ticketseverities'] && $focus->column_fields['ticketseverities'] != '')
			{
				$updatelog .= $app_strings['LOG_SEVERITY_CHANGE'].$focus->column_fields['ticketseverities'];
			}
			//Category change log
			if($old_category != $focus->column_fields['ticketcategories'] && $focus->column_fields['ticketcategories'] != '')
			{
				$updatelog .= $app_strings['LOG_CATEGORY_CHANGE'].$focus->column_fields['ticketcategories'];
			}

			$updatelog .= ' -- '.date('Y-m-d H:i:s').' '.$current_user->user_name.'--//--';
		}
		return $updatelog;
	}
