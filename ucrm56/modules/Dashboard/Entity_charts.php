...
function module_Chart_HomePageDashboard($userinfo) {
	...
	$calcountres = $adb->query("SELECT count(*) as count FROM vtiger_crmentity se INNER JOIN vtiger_activity act ON act.activityid = se.crmid
		WHERE se.deleted = 0 AND setype='Calendar' AND se.smownerid = $user_id AND ((act.status!='Completed' AND act.status!='Deferred') OR act.status IS NULL)
       ...
       if($frompage != '')
		{
			...
			//die;
			$count_len = 40*count(explode('::',$cnt_val));
			$length = ($count_len <= 400) ? 700 : (280+$count_len);
			
			return get_graph_homepg($cache_file_name,$html_imagename,$cnt_val,$name_val,$length,285,$left,$right,$top,$bottom,$title,$target_val,$graph_date,$urlstring,$test_target_val,$date_start,$end_date);
