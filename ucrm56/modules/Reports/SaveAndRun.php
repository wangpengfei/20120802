...
if($numOfRows > 0)
{
	...
	if(isPermitted($primarymodule,'index') == "yes" && $modules_permitted == true)
	{
		...
		//update by wqw 2010-08-16	
		$sshtml = $oReportRun->GenerateReport("HTML",$filterlist);

		if(is_array($sshtml)){
			$sql="select * from vtiger_reportsortcol where `sortcolid` =1 and `reportid` =$reportid";
			$sortby=$adb->query($sql)->fields['columnname'];
			
			$sortby_arr=explode(':',$sortby);
			
			$sortby=$sortby_arr[2];
			
		//echo 	$select=mb_substr($sshtml[2],6,mb_strpos($sshtml[2],'order by')-6);
		// modified  by wqw 2010-10-11 report error from salse order !!
			//	$sql='select sum(SUBSTRING_INDEX( SalesOrder_Total,"::",-1)) AS count,a.* from ('.$sshtml[2].') a group by '.$sortby;
	 		$sql='select count(*) AS count,a.* from ('.$sshtml[2].') a group by '.$sortby;

			$acc_result = $adb->pquery($sql,array());
			$norows = $adb->num_rows($acc_result);
			if($norows  > 0){
				global $aa;
				$i=0;
				$aa_key =@array_keys($aa);
				while($acc_row = $adb->fetch_array($acc_result))
				{
					$data_count[$i] = $acc_row['count'];
					$data_total += $acc_row['count'];
					
					if(is_array($aa) && in_array($acc_row[strtolower($sortby)],$aa_key)){
						$data_d[$i]=$aa[$acc_row[strtolower($sortby)]]?$aa[$acc_row[strtolower($sortby)]]:'-';
					}else{
						$data_d[$i] =  $acc_row[strtolower($sortby)]?$acc_row[strtolower($sortby)]:'-';
					}
					$data_d[$i] = getTranslatedString($data_d[$i], $primarymodule);
					$data_all[$data_d[$i]]=$data_count[$i];
					++$i;
				}
			}
			//ͼ����ʾ
			if(is_array($data_d)){
				$_SESSION['report_title']= getTranslatedString($oReportRun->reportname);
				$_SESSION['report_c']=$data_count;
				$_SESSION['report_d']=$data_d;
				$_SESSION['report_t']=$data_total;
				
				$sql="select * from vtiger_picklist where name ='$sortby_arr[1]'";
				if($adb->query($sql)->fields['picklistid']){
					require_once 'modules/PickList/PickListUtils.php';
					$roleid=$current_user->roleid;
					$picklistValues = getAssignedPicklistValues($sortby_arr[1], $roleid, $adb);
					//Ϊ�յ�����ŵ�һλ
					foreach($data_d as $key=>$val){
						if(!in_array($val,$picklistValues)){
							$d_c[]=$data_count[$key];
							$d_d[]=$val;
						}
					}
					...
					$_SESSION['report_c']=$d_c;
					$_SESSION['report_d']=$d_d;
				}
				$list_report_form->assign("graph_param", 1);
			}
		}
		//end
		$sshtml = array();
