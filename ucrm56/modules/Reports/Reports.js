...
function changeSteps1() 
{
	if(getObj('step5').style.display != 'none')
	{
		...
		if(conditionColumns.length == 0){
			conditionColumns = new Array(); 
			var e = document.getElementsByTagName('tr');
			for(i = 0; i < e.length; i++) {   
				if(e[i].getAttribute("name") == 'conditionColumn') {
					conditionColumns[conditionColumns.length] = e[i]; 
				}
			}
		}
		...
		for(var i=0;i < conditionColumns.length ; i++) {
			...
			if (!emptyCheck(columnId,alert_arr.Column,"text"))
				return false;
			if (!emptyCheck(comparatorId,selectedColumnLabel+alert_arr.Option,"text"))
				return false;
			...
			if(escapedOptions.indexOf(col[3]) == -1) {
				if(col[4] == 'T') {   
					...
					if(!re_dateValidate(datime[0],selectedColumnLabel+" ("+alert_arr.User_Date_Fomart+")","OTH"))
						return false
					if(datime.length > 1)	
					if(!re_patternValidate(datime[1],selectedColumnLabel+" ("+alert_arr.Time+")","TIMESECONDS"))
						return false
				}	
				else if(col[4] == 'D')
				{        
					if(!dateValidate(valueId,selectedColumnLabel+" ("+alert_arr.User_Date_Fomart+")","OTH"))
						return false
					if(extValueObject) {
						if(!dateValidate(extValueId,selectedColumnLabel+" ("+alert_arr.User_Date_Fomart+")","OTH"))
							return false
					}
				}else if(col[4] == 'I')
				{
					if(!intValidate(valueId,selectedColumnLabel+" ("+alert_arr.Integer+")"+i))           
						return false
				}else if(col[4] == 'N')
				{  
					if (!numValidate(valueId,selectedColumnLabel+" ("+alert_arr.Number+") ","any",true))
						return false
				}else if(col[4] == 'E')
				{
					if (!patternValidate(valueId,selectedColumnLabel+" ("+alert_arr.Email+")","EMAIL"))
						return false
				}
			}
		...
		if(!dateValidate("startdate",alert_arr.Start_Date,"D"))
        	        return false
	
		if(!dateValidate("enddate",alert_arr.End_Date,"D"))
        	        return false

		if(! dateComparison("startdate",alert_arr.Start_Date,"enddate",alert_arr.End_Date,'LE'))	
			return false;
		}
...
function fnLoadRepValues(tab1,tab2,block1,block2){
  ...
  //After taking a few characters in the string
...
//Select All Event 1
function checkEvent1(value,CheckId)
{

  var allCk=document.getElementsByName(CheckId);
  if(allCk[0].checked==true)   
	{
		allCk[1].checked=true;
		checkAll(value);		
	}
  if(allCk[0].checked==false)
	{
		allCk[1].checked=false;
		checkAllNo(value);
	}
}

//Select All Event 2
function checkEvent2(value,CheckId)
{

  var allCk=document.getElementsByName(CheckId);
  if(allCk[1].checked==true)   
	{
		allCk[0].checked=true;
		checkAll(value);		
	}
  if(allCk[1].checked==false)
	{
...
{
	for (var i=0;i<document.NewReport.length;i++ )
	{
		if (document.NewReport.elements[i].name.Right(5)==value)
		{
			document.NewReport.elements[i].checked=true;
		}
	}
}

//Clear All
function checkAllNo(value)
{
	for (var i=0;i<document.NewReport.length;i++ )
	{
		if (document.NewReport.elements[i].name.Right(5)==value)
		{
			document.NewReport.elements[i].checked=false;
		}
	}
}