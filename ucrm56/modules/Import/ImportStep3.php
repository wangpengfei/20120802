...
//*******************************************************************************
		...
		for($i=0; $i<$_SESSION['import_module_field_count']; $i++){
			if( intval($_REQUEST['colnum'.$i]) != -1 ){
				$new_col_pos_to_field[]= $_REQUEST['colnum'.$i];
				$new_firstrow[] = $firstrow[$i];
			}else{
				$keyarr[] = $i;
			}
		}
		
		$new_ret_field_count = count($new_col_pos_to_field);
		unset($new_datarows);	
		if(count($keyarr)!=0){
			foreach( $datarows as $key => $val ){		
				...
				$new_datarows[]=$new_val;
			}
		}else{
			$new_datarows = $datarows;
		}

		$res = InsertImportRecords($new_datarows,$new_datarows,$focus,$new_ret_field_count,$new_col_pos_to_field,$START,$totalnoofrows,vtlib_purify($_REQUEST['module']),$totalnoofrows,$skipped_record_count,$new_firstrow); 
