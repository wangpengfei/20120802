 ...
 */
function InsertImportRecords($rows,$rows1,$focus,$ret_field_count,$col_pos_to_field,$start,$recordcount,$module,$totalnoofrows,$skip_required_count,$firstrow='')
{
	global $current_user;
	global $adb;
	global $mod_strings;
	...
	if($start == 0)
	{
		...
		$_SESSION['rowscount'] = count($rows)+1;
	...
	{
		//判断分批导入时的数量
		$_SESSION['rows_insert']++;
		if($_SESSION['rows_insert']==$_SESSION['rowscount'])  break;
		...
		for($field_count = 0; $field_count < $ret_field_count; $field_count++)
		{
			...
			if ( isset( $col_pos_to_field[$field_count]) )
			{
				...
				{
					//Here we are assigning the user id in column fields, so in function assign_user (ImportLead.php and ImportProduct.php files) we should use the id instead of user name when query the user or we can use $focus->column_fields['smownerid'] = $users_groups_list[$row[$field_count]];
		...
		if($module == 'Leads' && $focus->column_fields['mobile']){
			$mobile = $focus->column_fields['mobile'];
			$mobilequery = "SELECT crmid,createdtime,user_name,last_name 
					  FROM vtiger_leadaddress,vtiger_crmentity 
					  LEFT JOIN vtiger_users on vtiger_crmentity.smownerid = vtiger_users.id
					  WHERE mobile =? and vtiger_leadaddress.leadaddressid = vtiger_crmentity.crmid and vtiger_crmentity.deleted != 1 ";
			$params = array($mobile);
			$mobileresult = $adb->pquery($mobilequery, $params);
			if($adb->num_rows($mobileresult) > 0)
			{
				$fail_messge .= $mobile.'&nbsp;&nbsp;'.getTranslatedString('LBL_MOBILE_EXIST',$module)."&nbsp;&nbsp;"
				.getTranslatedString('LBL_CREATED').': '.$adb->query_result($mobileresult,0,'createdtime').'&nbsp;&nbsp;'.getTranslatedString('LBL_ASSIGNED_TO').': '.$adb->query_result($mobileresult,0,'last_name').'['.$adb->query_result($mobileresult,0,'user_name')."] "."<br />";
		...
		if ($do_save)
		{
			...
			foreach($focus->importable_fields as $fieldname=>$uitype){
				...
				/*
				if($uitype == 15){
					$focus->picklist_fields[$fieldname];
					switch($focus->column_fields[$fieldname]){
						case getTranslatedString('Call','Calendar'):
							$searchstring = 'Call';
							break;
						case getTranslatedString('Meeting','Calendar'):
							$searchstring = 'Meeting';
							break;
						case getTranslatedString('Tasks'):
							$searchstring = 'Task';
							break;
					}
				}*/
	...
	if($end >= $totalnoofrows) {
		...
		/*if($imported_records == $totalnoofrows) {
		 ...
		 echo $mod_strings['LBL_SKIPPED_1'];
	...
	$aciton_url = "index.php?action=".$action."&module=".$module."&modulename=".$modulename."&startval=".$end."&recordcount=".$RECORDCOUNT."&noofrows=".$totalnoofrows."&skipped_record_count=".$skip_required_count."&parenttab=".vtlib_purify($_SESSION['import_parenttab'])."&dup_type=".$dup_check_type."&auto_type=".$auto_dup_type;
		
	$message_1 = "<font color='red'>".$fail_messge."</font>";
	
	$_SESSION['message'].=$message_1;	
	
	$message_2 = "<b>".$mod_strings['LBL_SUCCESS']."</b>"."<br><br>" .$mod_strings['LBL_SUCCESS_1']."  $imported_records " .$mod_strings['of'].' '.$totalnoofrows."<br><br>" .$mod_strings['LBL_SKIPPED_1']."  $skip_required_count <br><br>".$dup_info;
	
	$message = $message_2.$_SESSION['message'];
    
    
/*
    //add by tanbin
    if($out_arr!=''){
        require_once 'include/utils/function.export.php';
        ob_end_clean();
        createxls($out_arr);
    }
...
?>
<form  method="post" action="<?php echo $aciton_url; ?>" name="msg" >
