...
require_once('class.readexcel.php');
...
@session_unregister('rowscount');
@session_unregister('rows_insert');
@session_unregister('message');
$_SESSION['totalrows'] = '';
$_SESSION['recordcount'] = 250;
$_SESSION['startval'] = 0;
$_SESSION['rowscount']=0;
$_SESSION['rows_insert']=0;
$_SESSION['message']='';
...
$tmp_text_file_name= $import_dir. "IMPORT_".$current_user->id.".xlsx";
$tmp_file_name = $import_dir. "IMPORT_".$current_user->id;

move_uploaded_file($_FILES['userfile']['tmp_name'], $tmp_text_file_name);

//读取上传xls文件
$readexcel = new readerexcel($tmp_text_file_name);
$content = $readexcel->reader();


//写入文本文件
$str='","';
$cont_txt .="\"".implode($str,array_keys($content[0]))."\"\r\n";
foreach($content  as $key => $val){
	$cont_txt .= "\"".implode($str,$val)."\"\r\n";
}
...
if(isset($_REQUEST['module']) && $_REQUEST['module'] != '')
{
	...
	/*foreach($focus->importable_fields as $fieldname_tmp => $uitype_tmp){
		if($uitype_tmp == 15){
			$focus->picklist_fields[$fieldname_tmp] = getPickListValues($fieldname_tmp,'H1');
		}
	}*/
}
else
...
if (isset($secondrow)) { //Asha: Fix for ticket #4432
	...
	{
		if(strlen($val) > 20)
			$secondrow[$ind] = mb_substr(to_html($val),0,20,'UTF-8')." ..........";
		else
	...
	if (isset($thirdrow)) { //Asha: Fix for ticket #4432
		...
		{
			if(strlen($val) > 20)
				$thirdrow[$ind] = mb_substr(to_html($val),0,20,'UTF-8')." ..........";
			else
...
$map_list_combo .= "&nbsp;&nbsp;&nbsp;<span id='delete_mapping' style='visibility:hidden;'><a href='javascript:; deleteMapping();'>".$app_strings['LBL_DELETE']."</a></span>";
