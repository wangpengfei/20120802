...
for($i=0;$i<$no_of_picklists;$i++)
{
	...
	for($j=0;$j<$adb->num_rows($picklist_result2);$j++)
	{
		...
		$translate_table_picklist_values[$fieldname][$j] = getTranslatedString($adb->query_result($picklist_result2, $j, $fieldname),$_REQUEST['module']);
	}
}
...
foreach($field_to_pos as $fieldname => $ind)
{
	if(in_array($fieldname,$available_picklists))
	{
		...
		{
			if(!$datarows[$i][$ind]){
				$datarows[$i][$ind] = $converted_table_picklist_values[$fieldname][0];
			}
			$csv_picklist_values[$fieldname][$i] = $datarows[$i][$ind];
		}
...
foreach($csv_picklist_values as $fieldname => $temp_array)
{
    ...
    //add by wqw (2)
	//$datarows_key=$field_to_pos[$fieldname];
    //end
	...
	foreach($temp_array as $ind => $picklist_value)
	{
		...
		$key_tmp = array_search($pick_val,$translate_table_picklist_values[$fieldname]);
		if(!in_array($pick_val,$converted_table_picklist_values[$fieldname]) && $key_tmp !== false){
			$datarows[$ind][$picklist_pos[$fieldname]] = $table_picklist[$fieldname][$key_tmp];
		}
		elseif(!in_array($pick_val,$converted_table_picklist_values[$fieldname]) && $key_tmp === false)
		{			
